...
有关完整的 API 文档，请参阅 [TypeScript SDK 参考#/page/agent-sdk/typescript 和 [Python SDK 参考#/page/agent-sdk/python。
...
当代理生成[子代理#/page/agent-sdk/subagents时，三个结果级字段在计数内容上有所不同。使用 `modelUsage` 或 Python 中的 `model_usage` 进行整树令牌计数；`usage` 字段一旦发生嵌套就会低估。
...
要请求缓存写入的 1 小时 TTL，请设置 [`ENABLE_PROMPT_CACHING_1H`#/page/env-vars 环境变量。您可以在 shell 或容器环境中导出它，或通过 `options.env` 传递它。
...
* [TypeScript SDK 参考#/page/agent-sdk/typescript - 完整的 API 文档
* [SDK 概述#/page/agent-sdk/overview - SDK 入门
* [SDK 权限#/page/agent-sdk/permissions - 管理工具权限
