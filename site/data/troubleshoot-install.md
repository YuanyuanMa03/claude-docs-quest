...
如果安装失败或无法登录，请在下面找到您的错误。有关 Claude Code 正常工作后的运行时问题，请参阅 [Troubleshooting#/page/troubleshooting。有关配置问题（例如设置未应用或 hooks 未触发），请参阅 [Debug your configuration#/page/debug-your-config。
...
| `API Error: 500`、`529 Overloaded`、`429` 或上面未列出的其他 4xx 和 5xx 错误                             | 请参阅[错误参考#/page/errors                                                                                                          |

如果您的问题未列出，请按照下面的诊断检查来缩小原因范围。

<Tip>
  如果您宁愿完全跳过终端，[Claude Code Desktop 应用#/page/desktop-quickstart可让您通过图形界面安装和使用 Claude Code。为 [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) 或 [Windows](https://claude.com/download?utm_source=claude_code\&utm_medium=docs) 下载它，无需任何命令行设置即可开始编码。在 Linux 上，按照 [Linux 安装说明#/page/desktop-linux使用 apt 安装应用。
</Tip>
...
<Note>
  [VS Code 扩展#/page/vs-code不会将 `claude` 放在此位置。它在扩展目录内捆绑了一个私有的 CLI 副本，用于其自己的聊天面板，不会将其添加到 PATH。如果您仅安装了扩展，`~/.local/bin/claude` 将不存在。运行[独立安装#/page/setup以从终端使用 `claude`，然后继续下面的步骤。
</Note>
...
**如果 Git 已安装**但 Claude Code 找不到它，请在您的 [settings.json file#/page/settings 中设置路径：
...
如果您在 WSL 内使用 `npm install -g` 安装了 Claude Code，这些问题适用。如果您使用了 [native installer#/page/setup，请跳过此部分。
...
* **在代理后面**：企业代理可能干扰 API 请求。有关代理设置，请参阅 [network configuration#/page/network-config。
...
有关完整的提供商设置，请参阅 [Amazon Bedrock#/page/amazon-bedrock、[Google Cloud 的 Agent Platform#/page/google-vertex-ai 或 [Microsoft Foundry#/page/microsoft-foundry。
