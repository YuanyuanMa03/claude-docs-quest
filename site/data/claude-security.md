...
该插件也不同于 Claude Code 中已有的审查工具：[security guidance 插件#/page/security-guidance在 Claude 编写代码时审查代码，[`/security-review`](/docs/zh-CN/commands#all-commands) 对您的分支运行单次扫描，[Code Review#/page/code-review 审查拉取请求。有关这些层如何堆叠的信息，请参阅[该插件如何与其他安全工具配合](#how-the-plugin-fits-with-other-security-tools)。
...
* Claude Code v2.1.154 或更高版本，在付费计划上，用于扫描用来编排其代理的[动态工作流#/page/workflows。在 Pro 上，从 `/config` 中的"动态工作流"行启用它们。
...
<Steps>
  ...
  <Step title="阅读报告">
    扫描运行时，它会在每个阶段开始时报告，详细信息可在 [`/workflows`#/page/workflows 下获得。结果进入您的存储库中的时间戳目录，在[阅读扫描结果](#read-the-scan-results)中描述。
  </Step>
...
您不必从菜单开始：直接要求一个任务，作为命令的参数，例如 `/claude-security scan my branch`，或用纯语言，例如"scan commit abc1234"。该插件在[自动模式#/page/permission-modes中效果最佳，这允许扫描的代理在每一步都无需权限提示地进行；当任务开始时，该插件会提醒您如何启用它。
...
Claude Security 插件是深度扫描层，在纵深防御堆栈中，与[security guidance 插件#/page/security-guidance、[`/security-review`](/docs/zh-CN/commands#all-commands)、[Code Review#/page/code-review、托管的 [Claude Security](https://claude.com/product/claude-security) 产品和您现有的扫描器一起：

| 阶段      | 工具                                                                          | 覆盖内容                        |
| :------ | :-------------------------------------------------------------------------- | :-------------------------- |
| 在会话中    | [Security guidance 插件#/page/security-guidance                            | Claude 编写的代码中的常见漏洞，在同一会话中修复 |
...
| 在拉取请求上  | [Code Review#/page/code-review，Team 和 Enterprise 计划                      | 具有完整代码库上下文的多代理正确性和安全审查      |
...
* [Security guidance 插件#/page/security-guidance：在同一会话中，在 Claude 编写代码时捕获问题
* [Code Review#/page/code-review：设置 PR 时间多代理审查
* [Claude Security](https://claude.com/product/claude-security)：监控连接存储库的托管服务
* [Claude Code 安全#/page/security：Claude Code 如何处理信任、权限和保护措施
