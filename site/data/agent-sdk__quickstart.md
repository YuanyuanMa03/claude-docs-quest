...
<Steps>
  ...
  <Step title="设置你的 API 密钥">
    ...
    * **Microsoft Azure**：设置 `CLAUDE_CODE_USE_FOUNDRY=1` 环境变量并配置 Azure 凭证
    ...
    有关详细信息，请参阅 [Amazon Bedrock#/page/amazon-bedrock、[Claude Platform on AWS#/page/claude-platform-on-aws、[Google Cloud 的 Agent Platform#/page/google-vertex-ai 或 [Microsoft Foundry#/page/microsoft-foundry 的设置指南。
...
<Note>
  此示例使用流式传输来实时显示进度。如果你不需要实时输出（例如，对于后台作业或 CI 管道），你可以一次性收集所有消息。有关详细信息，请参阅[流式传输与单轮模式#/page/agent-sdk/streaming-vs-single-mode。
</Note>
...
<Note>
  如果你看到"API key not found"，请确保你已在运行代理的 shell 中设置了 `ANTHROPIC_API_KEY` 环境变量。SDK 不会自动加载 `.env` 文件。有关更多帮助，请参阅[完整故障排除指南#/page/troubleshooting。
</Note>
...
上面的示例使用 `acceptEdits` 模式，它自动批准文件操作，以便代理可以在没有交互式提示的情况下运行。如果你想提示用户批准，使用 `default` 模式并提供一个 [`canUseTool` 回调#/page/agent-sdk/user-input来收集用户输入。为了获得更多控制，请参阅[权限#/page/agent-sdk/permissions。
...
* **[权限#/page/agent-sdk/permissions**：控制你的代理可以做什么以及何时需要批准
* **[Hooks#/page/agent-sdk/hooks**：在工具调用之前或之后运行自定义代码
* **[会话#/page/agent-sdk/sessions**：构建维护上下文的多轮代理
* **[MCP 服务器#/page/agent-sdk/mcp**：连接到数据库、浏览器、API 和其他外部系统
* **[托管#/page/agent-sdk/hosting**：将代理部署到 Docker、云和 CI/CD
