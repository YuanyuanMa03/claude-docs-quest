...
了解更多：[运行 agent teams#/page/agent-teams
...
了解更多：[Claude Code 如何工作#/page/how-claude-code-works
...
了解更多：[将会话输出共享为 artifacts#/page/artifacts
...
了解更多：[Checkpointing#/page/checkpointing
...
了解更多：[The `.claude` directory#/page/claude-directory
...
了解更多：[Commands#/page/commands · [Skills#/page/skills
...
了解更多：[Explore the context window#/page/context-window
...
了解更多：[开始使用 hooks#/page/hooks-guide · [Hooks 参考#/page/hooks
...
了解更多：[服务器管理的设置#/page/server-managed-settings · [设置文件](/docs/zh-CN/settings#settings-files)
...
了解更多：[Model Context Protocol#/page/mcp
...
一种执行单个提示并退出而不进行对话会话的模式，使用 `-p` 或 `--print` 调用。用于 CI、脚本和管道。该运行仍然保存为可恢复的会话，除非您传递 `--no-session-persistence`。[Agent SDK#/page/agent-sdk/overview 是 Python 和 TypeScript 等效项。以前称为 headless mode。

了解更多：[以编程方式运行 Claude Code#/page/headless
...
了解更多：[Output styles#/page/output-styles
...
了解更多：[选择权限模式#/page/permission-modes
...
一个 skills、hooks、subagents 和 MCP servers 的包，打包为单个可安装单元。Plugin skills 命名为 `plugin-name:skill-name`，以便多个 plugins 共存。通过[市场#/page/plugin-marketplaces跨团队分发 plugins。

了解更多：[Plugins#/page/plugins
...
了解更多：[`.claude` directory#/page/claude-directory
...
了解更多：[Remote Control#/page/remote-control
...
了解更多：[Sandboxing#/page/sandboxing
...
了解更多：[使用 skills 扩展 Claude#/page/skills
...
了解更多：[创建自定义 subagents#/page/sub-agents
...
了解更多：[Claude 可用的工具#/page/tools-reference
...
一个会话知道工作实际完成而不仅仅是看起来合理的方式。您给 Claude 一个它可以运行的检查，例如测试套件、构建或屏幕截图比较，Claude 迭代直到检查通过，而不是在一次尝试后停止。验证循环是 [`/goal`#/page/goal、无人值守运行和[动态工作流#/page/workflows的先决条件：没有它，唯一决定代理完成的东西就是代理本身。
...
了解更多：[使用 git worktrees 运行并行会话#/page/worktrees
