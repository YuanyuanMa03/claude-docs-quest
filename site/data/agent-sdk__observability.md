...
本指南说明了 SDK 如何发出遥测数据、如何配置导出，以及如何在数据到达后端后对其进行标记和过滤。要直接从 SDK 响应流读取令牌使用情况和成本，而不是导出到后端，请参阅[跟踪成本和使用情况#/page/agent-sdk/cost-tracking。
...
有关完整的指标名称、事件名称和属性列表，请参阅 Claude Code [监控#/page/monitoring-usage参考。Agent SDK 发出相同的数据，因为它运行相同的 CLI。跨度名称列在下面的[读取代理跟踪](#read-agent-traces)中。
...
* **`claude_code.hook`：** 包装每个 [hook#/page/agent-sdk/hooks 执行。除了上述变量外，还需要详细的测试版跟踪（`ENABLE_BETA_TRACING_DETAILED=1` 和 `BETA_TRACING_ENDPOINT`）。
...
跨度默认携带 `session.id` 属性。当您对同一[会话#/page/agent-sdk/sessions进行多个 `query()` 调用时，在您的后端中按 `session.id` 过滤以将它们视为一个时间线。如果 `OTEL_METRICS_INCLUDE_SESSION_ID` 设置为假值，则省略该属性。
...
* [跟踪成本和使用情况#/page/agent-sdk/cost-tracking：从消息流读取令牌和成本数据，无需外部后端。
* [托管 Agent SDK#/page/agent-sdk/hosting：在容器中部署代理，您可以在环境级别设置 OpenTelemetry 变量。
* [监控#/page/monitoring-usage：CLI 发出的每个环境变量、指标和事件的完整参考。
