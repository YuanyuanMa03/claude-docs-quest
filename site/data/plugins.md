...
想要安装现有插件？请参阅[发现和安装插件#/page/discover-plugins。有关完整的技术规范，请参阅[插件参考#/page/plugins-reference。
...
<Note>
  如果你没有看到 `/plugin` 命令，请将 Claude Code 更新到最新版本。有关升级说明，请参阅[故障排除#/page/troubleshooting。
</Note>
...
<Tip>
  `--plugin-dir` 标志对开发和测试很有用。当你准备好与他人共享你的插件时，请参阅[创建和分发插件市场#/page/plugin-marketplaces。
</Tip>
...
| `settings.json`   | 插件根 | 启用插件时应用的默认[设置#/page/settings          |
...
<Note>
  **后续步骤**：准备好添加更多功能了吗？跳转到[开发更复杂的插件](#develop-more-complex-plugins)以添加 agents、hooks、MCP servers 和 LSP servers。有关所有插件组件的完整技术规范，请参阅[插件参考#/page/plugins-reference。
</Note>
...
插件可以包含 [Agent Skills#/page/skills 以扩展 Claude 的功能。Skills 是模型调用的：Claude 根据任务上下文自动使用它们。
...
安装插件后，运行 `/reload-plugins` 以加载 Skills。有关完整的 Skill 编写指南，包括渐进式披露和工具限制，请参阅 [Agent Skills#/page/skills。
...
设置 `agent` 激活插件的[自定义 agents#/page/sub-agents 之一作为主线程，应用其系统提示、工具限制和模型。这让插件在启用时通过改变 Claude Code 的默认行为方式。
...
3. **创建或使用市场**：通过 [plugin marketplaces#/page/plugin-marketplaces 分发以供安装
4. **与他人测试**：在更广泛分发之前让团队成员测试插件

一旦你的插件在市场中，其他人可以使用 [Discover and install plugins#/page/discover-plugins 中的说明安装它。要将插件保持在你的团队内部，请在 [private repository](/docs/zh-CN/plugin-marketplaces#private-repositories) 中托管市场。
...
如果 Anthropic 在官方市场中列出你的插件，你的 CLI 可以提示 Claude Code 用户安装它。请参阅 [Recommend your plugin from your CLI#/page/plugin-hints。

<Note>
  有关完整的技术规范、调试技术和分发策略，请参阅 [Plugins reference#/page/plugins-reference。
</Note>
...
* [发现和安装插件#/page/discover-plugins：浏览市场并安装插件
...
* [创建和分发市场#/page/plugin-marketplaces：打包和共享你的插件
* [插件参考#/page/plugins-reference：完整的技术规范
* 深入了解特定的插件组件：
