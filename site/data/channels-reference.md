...
要使用现有频道而不是构建一个，请参阅 [Channels#/page/channels。Telegram、Discord、iMessage 和 fakechat 包含在研究预览中。
...
要使您的频道可安装和可共享，请将其包装在[插件#/page/plugins中并将其发布到[市场#/page/plugin-marketplaces。用户使用 `/plugin install` 安装它，然后使用 `--channels plugin:<name>@<marketplace>` 按会话启用它。
...
* [Channels#/page/channels 安装和使用 Telegram、Discord、iMessage 或 fakechat 演示，以及为 Team 或 Enterprise 组织启用频道
* [工作频道实现](https://github.com/anthropics/claude-plugins-official/tree/main/external_plugins)用于具有配对流、回复工具和文件附件的完整服务器代码
* [MCP#/page/mcp 用于频道服务器实现的基础协议
* [Plugins#/page/plugins 打包您的频道，以便用户可以使用 `/plugin install` 安装它
