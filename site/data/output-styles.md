...
有关你的项目、约定或代码库的说明，请改用 [CLAUDE.md#/page/memory。
...
运行 `/config` 并选择**输出样式**从菜单中选择一种样式。你的选择会保存到[本地项目级别#/page/settings的 `.claude/settings.local.json`。
...
[Plugins#/page/plugins-reference 也可以在 `output-styles/` 目录中提供输出样式。
...
| [CLAUDE.md#/page/memory  | 在系统提示之后添加用户消息        | Claude 应该始终了解你的项目约定和代码库上下文 |
| `--append-system-prompt`    | 附加到系统提示而不删除任何内容      | 你想要一次性添加单个调用               |
| [Agents#/page/sub-agents | 使用自己的系统提示、模型和工具运行子代理 | 你想要一个单独作用域的辅助工具来完成专注的任务    |
| [Skills#/page/skills     | 在调用时或相关时加载特定于任务的说明   | 你有一个可重用的工作流                |

<h2 id="related-resources">
  相关资源
</h2>

* [Settings#/page/settings：`outputStyle` 字段所在的位置以及设置优先级的工作原理
* [Permission modes#/page/permission-modes：Proactive 样式与自动模式的比较方式
* [Plugins#/page/plugins：打包和分发输出样式以及 skills、hooks 和 agents
* [Debug your configuration#/page/debug-your-config：诊断为什么输出样式没有生效
