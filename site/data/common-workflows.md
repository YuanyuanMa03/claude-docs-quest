...
本页收集了日常开发的简短工作流程。有关提示和上下文管理的更高级指导，请参阅[最佳实践#/page/best-practices。
...
有关在 monorepo 或大型代码库中配置 Claude Code 的信息，请参阅 [Monorepos 和大型存储库#/page/large-codebases。
...
| 选项                                       | 运行位置              | 最适合                                                                                                               |
| :--------------------------------------- | :---------------- | :---------------------------------------------------------------------------------------------------------------- |
| [Routines#/page/routines              | Anthropic 管理的基础设施 | 即使您的计算机关闭也应该运行的任务。也可以在 API 调用或 GitHub 事件上触发，除了计划。在 [claude.ai/code/routines](https://claude.ai/code/routines) 配置。 |
| [桌面计划任务#/page/desktop-scheduled-tasks | 您的机器，通过桌面应用       | 需要直接访问本地文件、工具或未提交更改的任务。                                                                                           |
| [GitHub Actions#/page/github-actions  | 您的 CI 管道          | 与存储库事件（如打开的 PR）相关的任务，或应该与工作流配置一起存在的 cron 计划。                                                                      |
| [`/loop`#/page/scheduled-tasks        | 当前 CLI 会话         | 会话打开时的快速轮询。任务在您开始新对话时停止；`--resume` 和 `--continue` 恢复未过期的任务。                                                       |
...
这会恢复当前目录中最近的会话；如果还没有，它会打印 `No conversation found to continue` 并退出。使用 `claude --resume` 从列表中选择，或从运行中的会话内使用 `/resume`。有关命名、分支和完整选择器参考，请参阅[管理会话#/page/sessions。
...
在第二个终端中使用不同的名称运行相同的命令以启动隔离的并行会话。有关清理、`.worktreeinclude` 和非 git VCS 支持，请参阅 [Worktrees#/page/worktrees。要从一个屏幕而不是单独的终端监视并行会话，请参阅[后台代理#/page/agent-view。
...
subagent 在其自己的上下文窗口中读取文件并报告摘要。有关定义具有自己工具和提示的自定义代理，请参阅 [Subagents#/page/sub-agents。
...
有关输出格式、权限标志和扇出模式，请参阅[非交互模式#/page/headless。
