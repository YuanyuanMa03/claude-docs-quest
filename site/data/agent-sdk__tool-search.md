...
设置[`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS`#/page/env-vars会保持工具搜索关闭，`ENABLE_TOOL_SEARCH`无法覆盖它。该变量会删除`defer_loading`工具定义和`tool_reference`内容块所需的beta标头。

工具搜索适用于所有已注册的工具，无论它们来自远程MCP服务器还是[自定义SDK MCP服务器#/page/agent-sdk/custom-tools。使用`auto`时，阈值基于所有服务器上所有工具定义的组合大小。
...
有关完整的系统提示选项集，请参阅[修改系统提示#/page/agent-sdk/modifying-system-prompts。
...
* [连接MCP服务器#/page/agent-sdk/mcp：通过MCP服务器连接到外部工具
* [自定义工具#/page/agent-sdk/custom-tools：使用SDK MCP服务器构建您自己的工具
* [TypeScript SDK参考#/page/agent-sdk/typescript：完整API参考
* [Python SDK参考#/page/agent-sdk/python：完整API参考
