...
在 Windows 上，`~/.claude` 解析为 `%USERPROFILE%\.claude`。如果您设置了 [`CLAUDE_CONFIG_DIR`#/page/env-vars，此页面上的每个 `~/.claude` 路径都将位于该目录下。
...
| 文件                      | 位置                  | 用途                                                                                                                                                                 |
| ----------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `managed-settings.json` | 系统级别，因操作系统而异        | 企业强制执行的设置，您无法覆盖。请参阅[服务器管理的设置#/page/server-managed-settings。                                                                                                     |
...
| 您想要                   | 编辑                                      | 范围    | 参考                                                      |
| :-------------------- | :-------------------------------------- | :---- | :------------------------------------------------------ |
| 为 Claude 提供项目上下文和约定   | `CLAUDE.md`                             | 项目或全局 | [Memory#/page/memory                                 |
| 允许或阻止特定工具调用           | `settings.json` `permissions` 或 `hooks` | 项目或全局 | [Permissions#/page/permissions、[Hooks#/page/hooks |
| 在工具调用前后运行脚本           | `settings.json` `hooks`                 | 项目或全局 | [Hooks#/page/hooks                                   |
...
| 添加使用 `/name` 调用的提示或功能 | `skills/<name>/SKILL.md`                | 项目或全局 | [Skills#/page/skills                                 |
| 定义具有自己工具的专门 subagent  | `agents/*.md`                           | 项目或全局 | [Subagents#/page/sub-agents                          |
| 通过脚本编排许多 subagent     | `workflows/*.js`                        | 项目或全局 | [Dynamic workflows#/page/workflows                   |
| 通过 MCP 连接外部工具         | `.mcp.json`                             | 仅项目   | [MCP#/page/mcp                                       |
| 更改 Claude 格式化响应的方式    | `output-styles/*.md`                    | 项目或全局 | [Output styles#/page/output-styles                   |
...
<Note>
  ...
  * 您的组织部署的[托管设置#/page/server-managed-settings优先于所有内容
  * CLI 标志（如 `--permission-mode` 或 `--settings`）在该会话中覆盖 `settings.json`
  * 某些环境变量优先于其等效设置，但这会有所不同：检查[环境变量参考#/page/env-vars以了解每个变量
...
| 文件                                                  | 范围    | 提交 | 作用                                                           | 参考                                                                 |
| --------------------------------------------------- | ----- | -- | ------------------------------------------------------------ | ------------------------------------------------------------------ |
| [`CLAUDE.md`](#ce-claude-md)                        | 项目和全局 | ✓  | 每个会话加载的指令                                                    | [内存#/page/memory                                                |
| [`rules/*.md`](#ce-rules)                           | 项目和全局 | ✓  | 主题范围的指令，可选择路径门控                                              | [Rules](/docs/zh-CN/memory#organize-rules-with-claude/rules/)           |
| [`settings.json`](#ce-settings-json)                | 项目和全局 | ✓  | 权限、hooks、环境变量、模型默认值                                          | [设置#/page/settings                                              |
...
| [`skills/<name>/SKILL.md`](#ce-skills)              | 项目和全局 | ✓  | 可重用的提示，使用 `/name` 调用或自动调用                                    | [Skills#/page/skills                                            |
| [`commands/*.md`](#ce-commands)                     | 项目和全局 | ✓  | 单文件提示；与 skills 相同的机制                                         | [Skills#/page/skills                                            |
| [`output-styles/*.md`](#ce-output-styles)           | 项目和全局 | ✓  | 自定义系统提示部分                                                    | [输出样式#/page/output-styles                                       |
| [`agents/*.md`](#ce-agents)                         | 项目和全局 | ✓  | Subagent 定义及其自己的提示和工具                                        | [Subagents#/page/sub-agents                                     |
| [`workflows/*.js`](#ce-workflows)                   | 项目和全局 | ✓  | 由 Claude 编写并从 `/workflows` 保存的动态工作流脚本；每个文件都成为一个 `/<name>` 命令 | [动态工作流#/page/workflows                                          |
...
| [`keybindings.json`](#ce-keybindings)               | 仅全局   |    | 自定义快捷键                                                       | [快捷键#/page/keybindings                                          |
...
如果设置、hook 或文件未生效，请参阅[调试您的配置#/page/debug-your-config以获取检查命令和症状优先查找表。
...
| `projects/<project>/<session>/subagents/`    | [Subagent#/page/sub-agents 对话记录，当父会话记录过期时被删除                                                                               |
| `projects/<project>/<session>/tool-results/` | 大型工具输出溢出到单独的文件                                                                                                                |
| `file-history/<session>/`                    | Claude 更改的文件的编辑前快照，用于 [checkpoint 恢复#/page/checkpointing。保存最近 100 个 checkpoint 的快照；没有保留 checkpoint 引用的快照文件被删除，除了每个文件的第一个快照 |
...
| `remote-settings.json` | [服务器管理的设置#/page/server-managed-settings的缓存副本，用于您的组织。仅在您的组织配置了这些设置时才存在。在每次启动时刷新。 |
...
* 设置 [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`#/page/env-vars 环境变量以跳过在任何模式下写入记录和提示历史。在非交互模式下，您可以改为在 `-p` 旁边传递 `--no-session-persistence`，或在 Agent SDK 中设置 `persistSession: false`。
* 使用 [权限规则#/page/permissions 拒绝读取凭证文件
...
* [管理 Claude 的内存#/page/memory：编写和组织 CLAUDE.md、rules 和自动内存
* [配置设置#/page/settings：设置权限、hooks、环境变量和模型默认值
* [创建 skills#/page/skills：构建可重用的提示和工作流
* [配置 subagents#/page/sub-agents：定义具有自己上下文的专门代理
