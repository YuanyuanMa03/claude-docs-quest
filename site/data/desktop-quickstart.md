...
本页面将指导您安装应用并开始您的第一个会话。如果您已经设置完成，请参阅[使用 Claude Code Desktop#/page/desktop了解完整参考。
...
<Steps>
  <Step title="安装并登录">
    在 macOS 和 Windows 上，从上面的链接下载安装程序并运行它。在 Linux 上，请按照 [Claude Desktop on Linux#/page/desktop-linux 中的安装步骤进行操作。在 macOS 上从应用程序文件夹启动 Claude，在 Windows 上从开始菜单启动，或在 Linux 上从应用程序启动器启动，然后使用您的 Anthropic 账户登录。
  </Step>
...
桌面应用包含 Claude Code。您无需单独安装 Node.js 或 CLI。要从终端使用 `claude`，请单独安装 CLI。请参阅[开始使用 CLI#/page/quickstart。
...
<Steps>
  <Step title="选择环境和文件夹">
    ...
    * **Remote**：在 Anthropic 的云基础设施上运行会话，即使关闭应用也会继续。远程会话使用与 [Claude Code on the web#/page/claude-code-on-the-web 相同的基础设施。
    * **SSH**：通过 SSH 连接到远程机器，例如您自己的服务器、云虚拟机或开发容器。Desktop 在您第一次连接时会自动在远程机器上安装 Claude Code。
    * **WSL**（Windows）：在 [WSL 2 发行版#/page/desktop-wsl 内运行会话；Claude Code、工具和 git 在 Linux 端执行，使用本机路径。
  </Step>
...
您已经进行了第一次编辑。有关 Desktop 可以做的所有事情的完整参考，请参阅[使用 Claude Code Desktop#/page/desktop。以下是一些接下来可以尝试的事情。
...
**使用 skills 处理可重复的任务。** 输入 `/` 或点击 **+** → **Slash commands** 以浏览[内置命令#/page/commands、[自定义 skills#/page/skills 和插件 skills。Skills 是可重用的提示，您可以在需要时调用，例如代码审查清单或部署步骤。
...
**将 Claude 放在日程上。** 设置[计划任务#/page/desktop-scheduled-tasks以定期自动运行 Claude：每天早上进行代码审查、每周进行依赖审计，或从您连接的工具中提取信息的简报。
...
* [使用 Claude Code Desktop#/page/desktop：权限模式、并行会话、差异视图、连接器和企业配置
* [故障排除](/docs/zh-CN/desktop#troubleshooting)：常见错误和设置问题的解决方案
* [最佳实践#/page/best-practices：编写有效提示和充分利用 Claude Code 的提示
* [常见工作流#/page/common-workflows：调试、重构、测试等教程
