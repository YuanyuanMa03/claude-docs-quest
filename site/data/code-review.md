...
<Note>
  Code Review 处于研究预览阶段，仅适用于 [Team 和 Enterprise](https://claude.ai/admin-settings/claude-code) 订阅。对于启用了 [Zero Data Retention#/page/zero-data-retention 的组织，此功能不可用。
</Note>
...
要在您自己的 CI 基础设施中运行 Claude 而不是使用此托管服务，请参阅 [GitHub Actions#/page/github-actions 或 [GitLab CI/CD#/page/gitlab-ci-cd。对于自托管 GitHub 实例上的存储库，请参阅 [GitHub Enterprise Server#/page/github-enterprise-server。
...
<Steps>
  ...
  <Step title="安装 Claude GitHub App">
    ...
    * **Pull requests**：读写
    ...
    Code Review 使用对内容的读取访问权限和对 pull request 的写入访问权限。更广泛的权限集也支持 [GitHub Actions#/page/github-actions，如果您稍后启用的话。
  </Step>
...
Claude 在目录层次结构的每个级别读取 `CLAUDE.md` 文件，因此子目录的 `CLAUDE.md` 中的规则仅适用于该路径下的文件。有关 `CLAUDE.md` 如何工作的更多信息，请参阅[内存文档#/page/memory。
...
[`/code-review` 命令#/page/commands在您的终端中审查差异，无需安装 GitHub App。在任何 Claude Code 会话中运行它：它报告正确性错误和重用、简化和效率清理。默认情况下，本地审查涵盖您分支相对于其上游的提前提交加上工作树中的任何未提交更改。传递 `--comment` 以将发现作为内联 PR 评论发布，或传递 `--fix` 以在审查后将发现应用到您的工作树。
...
`/code-review ultra --fix` 在云中运行更深入的 [ultrareview#/page/ultrareview，然后在发现到达您的会话时将其应用到您的工作树。Ultrareview 使用其自己的范围：您当前的分支与存储库的默认分支，加上工作树中的任何未提交和暂存的更改。
...
* [Commands#/page/commands：在本地 Claude Code 会话中运行 `/code-review` 以在推送前检查差异
* [GitHub Actions#/page/github-actions：在您自己的 GitHub Actions 工作流中运行 Claude，以实现超越代码审查的自定义自动化
* [GitLab CI/CD#/page/gitlab-ci-cd：GitLab 管道的自托管 Claude 集成
* [Memory#/page/memory：`CLAUDE.md` 文件如何在 Claude Code 中工作
* [Analytics#/page/analytics：跟踪超越代码审查的 Claude Code 使用情况
