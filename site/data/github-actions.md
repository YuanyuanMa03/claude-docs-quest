...
Claude Code GitHub Actions 为您的 GitHub 工作流带来了 AI 驱动的自动化。只需在任何 PR 或 issue 中简单地提及 `@claude`，Claude 就可以分析您的代码、创建拉取请求、实现功能和修复错误 - 所有这些都遵循您项目的标准。如需在每个 PR 上自动发布评论而无需触发器，请参阅 [GitHub Code Review#/page/code-review。

<Note>
  Claude Code GitHub Actions 建立在 [Claude Agent SDK#/page/agent-sdk/overview 之上，该 SDK 支持将 Claude Code 以编程方式集成到您的应用程序中。您可以使用该 SDK 构建超越 GitHub Actions 的自定义自动化工作流。
</Note>
...
`prompt` 输入接受 [skill#/page/skills 调用以及纯文本：
...
* **Skills** - 直接从提示调用已安装的 [skills#/page/skills
...
| 参数                    | 描述                                              | 必需    |
| --------------------- | ----------------------------------------------- | ----- |
| `prompt`              | Claude 的说明（纯文本或 [skill#/page/skills 名称）      | 否\*   |
...
1. **CLAUDE.md**：在您的仓库根目录的 `CLAUDE.md` 文件中定义编码标准、审查标准和项目特定规则。Claude 在创建 PR 和响应请求时将遵循这些指南。查看我们的 [Memory 文档#/page/memory了解更多详情。
