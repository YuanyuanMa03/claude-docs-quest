...
某些终端默认阻止 OSC 52。iTerm2 会阻止它，直到您打开 Settings → General → Selection → Applications in terminal may access clipboard；在 iTerm2 中运行 [`/terminal-setup`#/page/terminal-config 会为您启用此功能。
...
设置 [`CLAUDE_CODE_ALT_SCREEN_FULL_REPAINT=1`#/page/env-vars 以在每一帧上重绘每个单元格，而不是发送增量更新。
...
在 Windows 上，Claude Code 已经为后台会话和 [agent view#/page/agent-view 自动启用完整重绘，因此您只需要为直接启动的交互式全屏会话设置该变量。
...
从 [agent view#/page/agent-view 或 `claude attach` 打开的后台会话始终使用全屏渲染。附加终端进入备用屏幕缓冲区以显示会话，经典渲染器在那里没有滚动缓冲或鼠标处理，因此 `tui` 设置和 `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN` 不适用于它们。
