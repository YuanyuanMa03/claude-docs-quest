...
| `app:toggleTodos`      | Ctrl+T | 切换 Claude 待办事项清单的可见性。这不是 [`/tasks`#/page/commands 后台任务视图 |
...
在启用[语音听写#/page/voice-dictation时，在 `Chat` 上下文中可用的操作：
...
在启用[全屏渲染#/page/fullscreen时，在 `Scroll` 上下文中可用的操作：
