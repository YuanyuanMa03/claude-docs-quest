...
* **云提供商**：如果您的组织使用 [Amazon Bedrock#/page/amazon-bedrock、[Google Cloud's Agent Platform#/page/google-vertex-ai 或 [Microsoft Foundry#/page/microsoft-foundry，请在运行 `claude` 之前设置所需的环境变量，或在登录提示符处选择 **3rd-party platform**，这将为 Bedrock 和 Vertex AI 启动交互式设置向导。不需要浏览器登录。
* **云网关**：如果您的组织运行自托管的 [Claude apps gateway#/page/claude-apps-gateway，请通过 `/login` 使用企业 SSO 登录。网关颁发的令牌是会话的唯一凭证。
...
* [Claude apps gateway#/page/claude-apps-gateway，一个自托管网关，使用您的 IdP 为开发人员签名，并将推理路由到您配置的云提供商
* [Amazon Bedrock#/page/amazon-bedrock
* [Google Cloud's Agent Platform#/page/google-vertex-ai
* [Microsoft Foundry#/page/microsoft-foundry
...
<Steps>
  <Step title="遵循提供商设置">
    遵循 [Amazon Bedrock 文档#/page/amazon-bedrock、[Google Cloud's Agent Platform 文档#/page/google-vertex-ai 或 [Microsoft Foundry 文档#/page/microsoft-foundry。
  </Step>

  <Step title="分发配置">
    将环境变量和生成云凭证的说明分发给您的用户。阅读有关如何 [在此处管理配置#/page/settings 的更多信息。
  </Step>
...
* **存储位置**：
  ...
  * Claude Code 通过 `/login` 和 `/logout` 管理 `.credentials.json`。要通过自定义 API 端点路由请求，请改为设置 [`ANTHROPIC_BASE_URL`#/page/env-vars 环境变量。
* **支持的身份验证类型**：Claude.ai 凭证、Claude API 凭证、Microsoft Foundry Auth、Bedrock Auth、Vertex Auth 和 [Claude apps gateway#/page/claude-apps-gateway 会话令牌。
...
对于运行无人值守的会话，提前续期最为重要。在 [agent view 中的后台会话#/page/agent-view 或 [Remote Control#/page/remote-control 会话一旦登录过期，就会停止进行，并且在您再次登录之前无法恢复。
...
1. 云提供商凭证，当设置了 `CLAUDE_CODE_USE_BEDROCK`、`CLAUDE_CODE_USE_VERTEX` 或 `CLAUDE_CODE_USE_FOUNDRY` 时。有关设置，请参阅 [第三方集成#/page/third-party-integrations。
2. `ANTHROPIC_AUTH_TOKEN` 环境变量。作为 `Authorization: Bearer` 标头发送。当通过 [LLM 网关或代理#/page/llm-gateway 进行路由时使用此选项，该网关或代理使用持有者令牌而不是 Anthropic API 密钥进行身份验证。
...
一个已签名的 [Claude apps gateway#/page/claude-apps-gateway 会话位于此列表之外：它是一个提供商选择，如 Amazon Bedrock 或 Google Cloud 的 Agent Platform，并且它优先于它们。当网关会话存在时，即使设置了 `CLAUDE_CODE_USE_BEDROCK`、`CLAUDE_CODE_USE_VERTEX` 或 `CLAUDE_CODE_USE_FOUNDRY`，CLI 也会使用网关令牌进行身份验证，上面的持有者令牌、API 密钥和 `apiKeyHelper` 条目不会被使用。
...
[Claude Code on the Web#/page/claude-code-on-the-web 始终使用您的订阅凭证。如果您在沙箱环境中设置 `ANTHROPIC_API_KEY` 或 `ANTHROPIC_AUTH_TOKEN`，它不会覆盖您的订阅凭证。
...
此令牌使用您的 Claude 订阅进行身份验证，需要 Pro、Max、Team 或 Enterprise 计划。它的范围仅限于推理，无法建立 [Remote Control#/page/remote-control 会话。
