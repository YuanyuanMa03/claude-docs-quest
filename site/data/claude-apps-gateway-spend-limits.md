...
支出限制限制了每个开发者在给定的一天、一周或一个月内通过你的 [Claude 应用网关#/page/claude-apps-gateway 可以花费的金额。当开发者超过他们的上限时，网关在他们的下一个请求上返回 `429`，并阻止他们直到该周期重置或管理员提高上限。使用支出限制为每个开发者、团队或整个组织设置一个共享凭证的上限。
