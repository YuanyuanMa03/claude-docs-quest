...
Worktrees 是运行 Claude 并行的几种方式之一。它们隔离文件编辑，而[子代理#/page/sub-agents和[代理团队#/page/agent-teams协调工作本身。请参阅[并行运行代理#/page/agents来比较这些方法，或跳到[使用 worktrees 隔离子代理](#isolate-subagents-with-worktrees)以同时使用 worktrees 和子代理。
...
您也可以在会话期间要求 Claude "在 worktree 中工作"，它将使用 [`EnterWorktree`#/page/tools-reference 工具创建一个。一旦进入 worktree，Claude 可以通过调用 `EnterWorktree` 并指定目标路径，直接切换到 `.claude/worktrees/` 下的另一个 worktree。之前的 worktree 保留在磁盘上不变。

进入存储库的 `.claude/worktrees/` 目录之外的路径首先会要求您的批准，因为它会移动会话的工作目录、写入访问权限和项目配置，例如 `CLAUDE.md` 和设置到该位置。`EnterWorktree` [权限规则#/page/permissions或选择"不再询问"不会抑制此提示；只有 `bypassPermissions` 模式会跳过它。在 v2.1.206 之前，Claude 可以进入任何现有的 worktree 路径而无需询问。

从 v2.1.198 开始，进入或退出 worktree 也会将会话记录重新定位到该目录的项目存储，与 [`/cd`#/page/commands 的方式相同，因此 `/desktop` 和 `--resume` 之后会在那里找到会话。由 [`WorktreeCreate` hook](#non-git-version-control) 创建的 Worktrees 被排除在外，并将记录保留在启动目录中。
...
在首次在目录中使用 `--worktree` 之前，请通过在该目录中运行一次 `claude` 来接受工作区信任对话框。如果尚未接受信任，`--worktree` 将以错误退出并提示您首先在目录中运行 `claude`。使用 `-p` 的非交互式运行会跳过[信任检查#/page/security，因此 `claude -p --worktree` 会在没有信任检查的情况下进行。
...
* [子代理#/page/sub-agents：在会话内将工作委派给隔离的代理
* [代理团队#/page/agent-teams：自动协调多个 Claude 会话
* [管理会话#/page/sessions：命名、恢复和在对话之间切换
