...
有关每个选项上可用功能的逐项细分，请参阅[功能可用性#/page/feature-availability。
...
* [Claude 应用网关#/page/claude-apps-gateway，一个自托管网关，在 Amazon Bedrock、Claude Platform on AWS、Google Cloud's Agent Platform、Microsoft Foundry 或 Anthropic API 前面添加 IdP 登录
* [Amazon Bedrock#/page/amazon-bedrock
* [Claude Platform on AWS#/page/claude-platform-on-aws
...
* **企业代理**：通过 HTTP/HTTPS 代理路由流量。如果您的组织要求所有出站流量通过代理服务器以进行安全监控、合规性或网络策略执行，请使用此选项。使用 `HTTPS_PROXY` 或 `HTTP_PROXY` 环境变量进行配置。在[企业网络配置#/page/network-config中了解更多。
* **LLM 网关**：位于 Claude Code 和云提供商之间的服务，用于处理身份验证和路由。如果您需要跨团队的集中使用情况跟踪、自定义速率限制或预算或集中身份验证管理，请使用此选项。使用 `ANTHROPIC_BASE_URL`、`ANTHROPIC_BEDROCK_BASE_URL`、`ANTHROPIC_AWS_BASE_URL`、`ANTHROPIC_VERTEX_BASE_URL` 或 `ANTHROPIC_FOUNDRY_BASE_URL` 环境变量进行配置。在[LLM 网关#/page/llm-gateway中了解更多。

以下示例显示在 shell 或 shell 配置文件（`.bashrc`、`.zshrc`）中设置的环境变量。有关其他配置方法，请参阅[设置#/page/settings。
...
<Tabs>
  <Tab title="企业代理">
    通过设置以下[环境变量#/page/env-vars，将 Amazon Bedrock 流量路由通过您的企业代理：
  ...
  <Tab title="LLM 网关">
    通过设置以下[环境变量#/page/env-vars，将 Amazon Bedrock 流量路由通过您的 LLM 网关：
...
<Tabs>
  <Tab title="企业代理">
    通过设置以下[环境变量#/page/env-vars，将 Microsoft Foundry 流量路由通过您的企业代理：
  ...
  <Tab title="LLM 网关">
    通过设置以下[环境变量#/page/env-vars，将 Microsoft Foundry 流量路由通过您的 LLM 网关：
...
<Tabs>
  <Tab title="企业代理">
    通过设置以下[环境变量#/page/env-vars，将 Google Cloud's Agent Platform 流量路由通过您的企业代理：
  ...
  <Tab title="LLM 网关">
    通过设置以下[环境变量#/page/env-vars，将 Google Cloud's Agent Platform 流量路由通过您的 LLM 网关：
...
在[内存和 CLAUDE.md 文件#/page/memory中了解更多。
...
如果您通过 [Amazon Bedrock#/page/amazon-bedrock、[Google Cloud 的 Agent Platform#/page/google-vertex-ai、[Microsoft Foundry#/page/microsoft-foundry 或 [Claude Platform on AWS#/page/claude-platform-on-aws 部署，请使用 `ANTHROPIC_DEFAULT_FABLE_MODEL`、`ANTHROPIC_DEFAULT_OPUS_MODEL`、`ANTHROPIC_DEFAULT_SONNET_MODEL` 和 `ANTHROPIC_DEFAULT_HAIKU_MODEL` 固定特定模型版本。如果不固定，模型别名会解析为 Claude Code 为该提供商内置的默认值，这可能滞后于最新版本，并且可能尚未在您的账户中启用。固定让您可以控制用户何时迁移到新模型。有关每个提供商在默认值不可用时的行为，请参阅[模型配置](/docs/zh-CN/model-config#pin-models-for-third-party-deployments)。

<h3 id="configure-security-policies">
  配置安全策略
</h3>

安全团队可以配置托管权限，定义 Claude Code 允许和不允许做什么，这不能被本地配置覆盖。[了解更多#/page/security。

<h3 id="leverage-mcp-for-integrations">
  利用 MCP 进行集成
</h3>

MCP 是为 Claude Code 提供更多信息的好方法，例如连接到票证管理系统或错误日志。我们建议一个中央团队配置 MCP servers 并将 `.mcp.json` 配置检入代码库，以便所有用户受益。[了解更多#/page/mcp。
...
1. **向您的团队推出**：分享安装说明，让团队成员[安装 Claude Code#/page/setup 并使用其凭证进行身份验证。
2. **设置共享配置**：在您的存储库中创建 [CLAUDE.md 文件#/page/memory，以帮助 Claude Code 理解您的代码库和编码标准。
3. **配置权限**：查看[安全设置#/page/security以定义 Claude Code 在您的环境中可以和不能做什么。
