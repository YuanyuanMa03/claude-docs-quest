...
要编写你的第一个配置，请从[快速入门](/docs/zh-CN/claude-apps-gateway#quickstart)开始，它构建一个最小的工作配置并运行它。一旦你有了满意的配置，[部署指南#/page/claude-apps-gateway-deploy涵盖了在 Kubernetes、Cloud Run 或你自己的平台上容器化和托管它。
...
对于网关替换或前置的客户端 Bedrock 部署，请参阅 [Amazon Bedrock 上的 Claude Code#/page/amazon-bedrock。网关端上游：
...
对于同一平台的客户端部署，请参阅 [Claude Platform on AWS 上的 Claude Code#/page/claude-platform-on-aws。网关端上游：
...
对于等效的客户端设置，请参阅 [Google Cloud 上的 Claude Code#/page/google-vertex-ai。网关端上游：
...
对于客户端 Foundry 部署，请参阅 [Microsoft Foundry 上的 Claude Code#/page/microsoft-foundry。网关端上游：
...
可选。启用 `/v1/organizations/spend_limits`，它镜像 Anthropic 的公共 Admin API，以及 `/v1/messages` 上的每开发者支出强制。有关如何设置和强制执行上限的信息，请参阅[支出限制#/page/claude-apps-gateway-spend-limits；本部分涵盖打开该功能和调整它的 `gateway.yaml` 键。
...
<Note>
  ...
  在真实来源处运行用户和组生命周期管理，这是你的 IdP 的本地 SCIM 配置或专用身份治理平台。那里管理的成员身份和取消配置通过令牌自动流入网关。如果你想要 Claude 帐户本身的 SCIM 配置，这是一个 [Claude for Enterprise#/page/admin-setup 功能。
...
| `permissions.allow` / `.deny`              | CLI      | 工具和命令规则。请参阅[权限#/page/permissions。                                                                                                                              |
...
* `sandbox.bwrapPath` 和 `sandbox.socatPath`：[沙箱#/page/sandboxing助手二进制文件的文件系统路径
* [`forceRemoteSettingsRefresh`#/page/server-managed-settings：阻止启动直到远程托管设置被新鲜获取，因此 MDM 或文件策略设置它被尊重，即使缺少该键的缓存远程有效负载是最高优先级来源
...
CLI 通过 HTTP 指标、日志和（启用时）跟踪将 OpenTelemetry Protocol (OTLP) 发送到网关，网关逐字中继到每个配置的目标。有关 CLI 发出的指标和事件，请参阅[监控使用#/page/monitoring-usage。
...
* [Claude 应用网关概述#/page/claude-apps-gateway：快速入门和开发者连接
* [部署指南#/page/claude-apps-gateway-deploy：IdP 设置、容器镜像、Kubernetes 和 Cloud Run，以及操作
* [支出限制#/page/claude-apps-gateway-spend-limits：每开发者上限和 Admin API
