...
* **一个本地麦克风**：语音听写在远程环境中不起作用，例如[网络上的 Claude Code#/page/claude-code-on-the-web或 SSH 会话。
...
转录不消耗 Claude 消息或令牌，也不计入 `/usage` 中显示的限制。有关 Anthropic 如何处理你的数据，请参阅[数据使用#/page/data-usage。
...
Claude Code [VS Code 扩展#/page/vs-code也支持语音听写，具有相同的 Claude.ai 账户要求。它在 VS Code Remote 会话中不可用，包括 SSH、Dev Containers 和 Codespaces，因为麦克风在你的本地机器上，而扩展在远程主机上运行。
...
语音听写在会话之间持续。直接在你的[用户设置文件#/page/settings中设置它，而不是运行 `/voice`：
...
启用语音听写时，当提示词为空时，输入页脚会显示 `hold space to speak` 提示。提示文本反映你当前的 `voice:pushToTalk` 快捷键绑定，如果你[重新绑定听写键](#rebind-the-dictation-key)，它会更新。提示文本在两种模式中都相同，如果你配置了[自定义状态行#/page/statusline，则不会显示。
...
语音听写使用与控制 Claude 响应语言相同的[`language` 设置#/page/settings。如果该设置为空，听写默认为英语。在 VS Code 扩展中，如果 `language` 为空，听写在默认为英语之前使用 VS Code 的 `accessibility.voice.speechLanguage` 设置。
...
听写键在 `Chat` 上下文中绑定到 `voice:pushToTalk`，默认为 `Space`。相同的绑定控制按住和点击模式。在 [`~/.claude/keybindings.json`#/page/keybindings 中重新绑定它：
...
某些键不会传递到终端应用程序，根本无法绑定。例如，如果你尝试绑定 `Caps Lock`，会显示错误。有关完整的快捷键语法和保留快捷键列表，请参阅[自定义键盘快捷键#/page/keybindings。
...
* [自定义键盘快捷键#/page/keybindings：重新绑定 `voice:pushToTalk` 和其他 CLI 键盘操作
* [配置设置#/page/settings：`voice`、`language` 和其他设置键的完整参考
* [交互模式#/page/interactive-mode：键盘快捷键、输入模式和会话控制
* [命令#/page/commands：`/voice`、`/config` 和所有其他命令的参考
