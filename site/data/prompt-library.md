...
这些提示词来自各种 Anthropic 指南，包括[常见工作流#/page/common-workflows、[最佳实践#/page/best-practices和[Anthropic 团队如何使用 Claude Code](https://claude.com/blog/how-anthropic-teams-use-claude-code)。它们是起点而不是脚本。打开任何提示词下的**为什么这样做有效**来查看其背后的模式，这样你可以编写自己的提示词。
...
export const labels = {
  ...
  needsLabel: "需要",
  needs: {
    tracker: "你的问题跟踪器添加为 [claude.ai 连接器](/docs/zh-CN/mcp#use-mcp-servers-from-claude-ai) 或 [MCP 服务器#/page/mcp。",
    gh: "[gh CLI](https://cli.github.com) 已认证，或 GitHub 添加为 [claude.ai 连接器](/docs/zh-CN/mcp#use-mcp-servers-from-claude-ai)。",
    browser: "Claude 能够呈现和截图结果的方式。[桌面应用](/docs/zh-CN/desktop#preview-your-app)内置了此功能。在终端中，安装 [Chrome 扩展#/page/chrome或 Playwright [MCP#/page/mcp 服务器。",
    db: "你的数据仓库或日志存储添加为 [claude.ai 连接器](/docs/zh-CN/mcp#use-mcp-servers-from-claude-ai) 或 [MCP 服务器#/page/mcp。"
  }
};
...
export const text = {
  ...
  "turn-a-meeting-into": {
    title: "将会议转变为工单",
    teaches: "跳过转录步骤。Claude 从非结构化输入中提取行动项，并通过 [MCP#/page/mcp 直接将其写入你的跟踪器，所以你审查工单，而不是转录。",
  ...
  "run-a-security-review": {
    title: "使用子代理运行安全审查",
    teaches: "[子代理#/page/sub-agents在其自己的上下文窗口中运行审计并报告回摘要，所以长安全审查不会填满你的主会话。内置的通用子代理无需额外设置即可处理此问题。",
  ...
  "turn-a-correction-into": {
    title: "将更正转变为规则",
    teaches: "聊天中的更正不与你的团队共享。项目的 [CLAUDE.md#/page/memory 中的规则在你提交后共享，Claude 在每个会话开始时读取它。",
  ...
  "turn-a-recurring-task": {
    title: "将重复任务转变为技能",
    teaches: "命名步骤一次；将其重用为命令。Claude 编写任何团队成员都可以运行的 [技能#/page/skills。"
  },
  "add-a-hook-for": {
    title: "为重复行为添加钩子",
    teaches: "钩子使行为自动化，而不是你必须记住要求的东西。描述触发器和操作，Claude 编写 [钩子#/page/hooks 配置。"
  },
  "connect-a-tool-with": {
    title: "使用 MCP 连接工具",
    teaches: "连接源一次，而不是每个会话粘贴数据。在 [MCP#/page/mcp 设置后，当你询问它时，Claude 直接从工具读取。"
  },
  "capture-what-to-remember": {
    title: "捕获下次要记住的内容",
    teaches: "在你忘记之前询问。Claude 知道它在这个会话中必须弄清楚什么，并提议 [CLAUDE.md#/page/memory 条目，以便下一个会话以该上下文开始。"
  }
};
...
**说出你想要答案的方式。** 命名格式、长度或受众，以便解释适合你将如何使用它。要使格式成为每个响应的默认值，请设置 [输出样式#/page/output-styles。

```text theme={null}
explain how the payment retry logic works as an HTML page with a diagram, then open it in my browser
```

有关每个模式的更多信息，请参阅[最佳实践#/page/best-practices。
...
* [常见工作流#/page/common-workflows：核心任务的分步指南
* [最佳实践#/page/best-practices：提示词模式和项目设置
...
此页面上的提示词是起点。一旦一个对你的项目有效，下一步是使其可重复：将其保存为 [技能#/page/skills，以便团队中的任何人都可以将其作为 `/command` 运行，并在 [CLAUDE.md#/page/memory 中记录 Claude 学到的约定，以便每个会话都以该上下文开始，而不是 Claude 重新学习它。对于更大或更危险的更改，[Plan Mode](/docs/zh-CN/permission-modes#analyze-before-you-edit-with-plan-mode) 在任何编辑发生前显示文件列表。

如果你在团队中引入 Claude Code，请参阅[管理#/page/admin-setup以获取托管设置和策略，以及[成本和使用#/page/costs以了解此工作如何在你的计划上计费。
