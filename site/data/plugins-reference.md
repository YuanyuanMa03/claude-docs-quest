...
<Tip>
  想要安装插件？请参阅[发现和安装插件#/page/discover-plugins。如需创建插件，请参阅[Plugins#/page/plugins。如需分发插件，请参阅[Plugin marketplaces#/page/plugin-marketplaces。
</Tip>
...
有关完整详情，请参阅 [Subagents#/page/sub-agents。
...
Plugin hooks 响应与 [用户定义的 hooks#/page/hooks 相同的生命周期事件：
...
* `mcp_tool`：在配置的 [MCP server#/page/mcp 上调用工具
...
| 您拥有的                                          | 它是什么                                                     |
| :-------------------------------------------- | :------------------------------------------------------- |
| `<skills-dir>/foo/SKILL.md` 没有清单              | 一个名为 `foo` 的普通 [skill#/page/skills                    |
...
| `<cwd>/.claude/skills/` | 项目 | 仅在您接受该文件夹的工作区 [trust dialog#/page/settings 后 |

项目范围的 plugin 被检入存储库，并到达克隆它的每个协作者。因为该内容来自存储库而不是来自您，它仅在与 `.claude/settings.json` 相同的信任门后加载，并且运行代码的组件受到进一步限制：

* 它声明的 MCP servers 通过与项目 `.mcp.json` 相同的 [per-server approval#/page/mcp
...
| `dependencies`          | array                 | 此 plugin 需要的其他 plugins，可选择带有 semver 版本约束。请参阅[约束 plugin 依赖版本#/page/plugin-dependencies               | `[{ "name": "secrets-vault", "version": "~2.1.0" }]` |
...
| **Settings**      | `settings.json`              | 启用 plugin 时应用的默认配置。目前仅支持 [`agent`#/page/sub-agents 和 [`subagentStatusLine`](/docs/zh-CN/statusline#subagent-status-lines) 键 |
...
| `channel`      | 一个基于 MCP 的 [channel#/page/channels：一个 stdio server (`server.ts`)、它的 `.mcp.json` 和一个 `package.json` |
...
删除不再被任何已安装 plugin 需要的自动安装 plugin 依赖项。Claude Code 为满足另一个 plugin 的 [`dependencies`#/page/plugin-dependencies 字段而引入的依赖项将被删除；您直接安装的 plugin 永远不会被触及。
...
启用已禁用的 plugin。如果 plugin 声明了[依赖项#/page/plugin-dependencies，Claude Code 会在同一范围内以传递方式启用它们，当依赖项未安装时命令会失败。
...
* [Plugins#/page/plugins - 教程和实际用法
* [Plugin marketplaces#/page/plugin-marketplaces - 创建和管理市场
* [Skills#/page/skills - Skill 开发详情
* [Subagents#/page/sub-agents - Agent 配置和功能
* [Hooks#/page/hooks - 事件处理和自动化
* [MCP#/page/mcp - 外部工具集成
* [Settings#/page/settings - Plugins 的配置选项
