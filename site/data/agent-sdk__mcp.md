...
直接在应用程序代码中定义自定义工具，而不是运行单独的服务器进程。有关实现详情，请参阅 [自定义工具指南#/page/agent-sdk/custom-tools。
...
工具搜索默认启用。有关配置选项和详情，请参阅 [工具搜索#/page/agent-sdk/tool-search。

有关更多详情，包括最佳实践和将工具搜索与自定义 SDK 工具一起使用，请参阅 [工具搜索指南#/page/agent-sdk/tool-search。
...
MCP 服务器连接默认超时为 30 秒。如果您的服务器需要更长时间才能启动，连接将失败。使用 [`MCP_TIMEOUT`#/page/env-vars 环境变量提高限制，单位为毫秒。对于需要更多启动时间的服务器，还应考虑：
...
SDK 应用与 Claude Code 相同的 MCP 输出限制。当工具结果大于 25,000 令牌时，完整输出被保存到文件，工具结果被替换为错误消息，该消息命名文件路径，以便代理可以分部分读取输出。使用 [`MAX_MCP_OUTPUT_TOKENS`#/page/env-vars 环境变量提高限制。有关完整行为（包括服务器如何声明更高的每工具限制），请参阅 [MCP 输出限制和警告](/docs/zh-CN/mcp#mcp-output-limits-and-warnings)。

<h2 id="related-resources">
  相关资源
</h2>

* **[自定义工具指南#/page/agent-sdk/custom-tools**：构建您自己的 MCP 服务器，与您的 SDK 应用程序在进程中运行
* **[权限#/page/agent-sdk/permissions**：使用 `allowedTools` 和 `disallowedTools` 控制您的代理可以使用哪些 MCP 工具
...
* **[TypeScript SDK 参考#/page/agent-sdk/typescript**：完整的 API 参考，包括 MCP 配置选项
* **[Python SDK 参考#/page/agent-sdk/python**：完整的 API 参考，包括 MCP 配置选项
