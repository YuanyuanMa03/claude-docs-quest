...
输出样式、`append` 和自定义提示词字符串各自直接改变系统提示词。CLAUDE.md 采用不同的方式：SDK 读取它并将其内容作为项目上下文注入到对话中，而不是注入到系统提示词中，因此它与你选择的任何系统提示词一起塑造行为。[Skills#/page/agent-sdk/skills、[hooks#/page/agent-sdk/hooks 和 [permissions#/page/agent-sdk/permissions 也在系统提示词之外塑造行为，并在各自的页面上介绍。

<h3 id="claude-md-files-for-project-level-instructions">
  CLAUDE.md 文件用于项目级指令
</h3>

CLAUDE.md 文件为 Claude 提供持久的项目上下文和指令。SDK 将其内容注入到对话中，而不是注入到系统提示词中，因此它们可以与任何系统提示词配置一起工作。关于在 CLAUDE.md 中放什么、在哪里放置它以及如何编写有效的指令，请参阅 [Claude 如何记住你的项目#/page/memory。本节涵盖 SDK 特定的内容：CLAUDE.md 如何加载。
...
对于非交互式 CLI 模式中的等效标志，请参阅 [`--exclude-dynamic-system-prompt-sections`#/page/cli-reference。
...
* [输出样式#/page/output-styles：为 CLI 创建、管理和共享输出样式，包括文件格式和存储位置
* [Claude 如何记住您的项目#/page/memory：CLAUDE.md 中应放入的内容、放置位置以及如何编写有效的项目说明
* [TypeScript SDK 参考#/page/agent-sdk/typescript：完整的 `Options` 类型，包括 `systemPrompt`、`settingSources` 和 `settings`
* [Python SDK 参考#/page/agent-sdk/python：完整的 `ClaudeAgentOptions` 类型，包括 `system_prompt` 和 `setting_sources`
* [Settings#/page/settings：`settings.json` 参考，包括输出样式和其他配置的存储位置
