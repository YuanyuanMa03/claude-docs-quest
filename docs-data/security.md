...
有关详细的权限配置，请参阅 [Permissions#/page/permissions。
...
* **沙箱化 bash 工具**：使用文件系统和网络隔离的 [Sandbox#/page/sandboxing bash 命令，减少权限提示同时保持安全性。使用 `/sandbox` 启用以定义 Claude Code 可以自主工作的边界
...
使用 [Claude Code on the web#/page/claude-code-on-the-web 时，会实施额外的安全控制：
...
* 在批准前审查所有建议的更改
* 为敏感存储库使用项目特定的权限设置
* 考虑使用 [dev containers#/page/devcontainer 以获得额外隔离
...
* 通过版本控制共享批准的权限配置
* 培训团队成员了解安全最佳实践
* 通过 [OpenTelemetry metrics#/page/monitoring-usage 监控 Claude Code 使用情况
...
* [Security guidance plugin#/page/security-guidance：让 Claude 在会话期间审查和修复其自身代码更改中的漏洞
* [Sandbox environments#/page/sandbox-environments：比较隔离方法并为您的威胁模型选择一个
* [Sandboxing#/page/sandboxing：Bash 命令的文件系统和网络隔离
* [Permissions#/page/permissions：配置权限和访问控制
* [Monitoring usage#/page/monitoring-usage：跟踪和审计 Claude Code 活动
* [Development containers#/page/devcontainer：安全、隔离的环境
