...
当终端文本不是 Claude 生成的内容的合适媒介时，请使用 artifact：输出更容易查看和交互，而不是逐行阅读。Claude 从您的会话可以访问的任何内容构建页面，包括您的代码库和通过您的 [连接工具#/page/mcp 拉取的数据，因此页面可以显示需要段落才能描述的内容。例如，要求 Claude：
...
从 Claude Code v2.1.183 开始，Claude 在构建 artifact 时应用内置设计技能，因此页面获得深思熟虑的调色板、排版和布局，无需额外提示。该技能还在选择自己的设计之前查找项目中的现有设计系统。要保持 artifacts 与您产品的品牌一致，请在 Claude 可以找到的地方记录您的设计令牌，例如项目的 [CLAUDE.md#/page/memory 或存储库中的主题文件：
...
| 身份验证  | 会话由 claude.ai 账户支持：在 CLI 或桌面应用中使用 `/login` 登录。Claude Tag 会话通过代理的身份登录，因此不需要任何步骤。使用 API 密钥、[网关令牌#/page/llm-gateway 或云提供商凭证的会话无法发布。                                                                                                                                                                                               |
| 模型提供商 | Anthropic API。在 [Amazon Bedrock#/page/amazon-bedrock、[Google Cloud 的 Agent Platform#/page/google-vertex-ai 或 [Microsoft Foundry#/page/microsoft-foundry 上不可用。                                                                                                                                                            |
| 组织策略  | 客户管理的加密密钥 (CMEK)、HIPAA 和 [零数据保留#/page/zero-data-retention 未为组织启用。                                                                                                                                                                                                                                                              |
| 表面    | Claude Code CLI 版本 2.1.183 或更高版本，或 Claude 桌面应用版本 1.13576.0 或更高版本。当 Claude Tag 和 artifacts 都为组织启用时，[Claude Tag](https://claude.com/docs/claude-tag/overview) 会话也可以发布 artifacts。在 [Agent SDK#/page/agent-sdk/overview、GitHub Action 和 MCP-server 上下文中默认关闭，以及当设置 [`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`#/page/env-vars 时。 |
...
| 方法                         | 设置                                  |
| :------------------------- | :---------------------------------- |
| [设置文件#/page/settings    | `"disableArtifact": true`           |
| [环境变量#/page/env-vars    | `CLAUDE_CODE_DISABLE_ARTIFACT=1`    |
| [权限规则#/page/permissions | 将 `Artifact` 添加到 `permissions.deny` |
...
* 浏览与 artifacts 配对的 [提示模式和工作流#/page/prompt-library
* 将您重复使用的 artifact 提示转换为 [skill#/page/skills，以便您可以将其作为命令调用
* [连接 MCP 服务器#/page/mcp，以便 Claude 可以在构建页面时将数据拉入 artifact
