...
<Note>
  会话保持**对话**，而不是文件系统。要快照和还原代理所做的文件更改，请使用[文件检查点#/page/agent-sdk/file-checkpointing。
</Note>
...
您需要多少会话处理取决于应用的形状。当您发送应该共享上下文的多个提示时，会话管理就会发挥作用。在单个 `query()` 调用中，代理已经根据需要进行了尽可能多的轮次，权限提示和 `AskUserQuestion` 是[在循环中处理的#/page/agent-sdk/user-input（它们不会结束调用）。
...
<Note>
  实验性的 [V2 会话 API#/page/agent-sdk/typescript-v2-preview（提供了带有 `send` / `stream` 模式的 `createSession()`）已在 TypeScript Agent SDK 0.3.142 中移除。使用 `query()` 函数和本页面上描述的会话选项。
</Note>
...
要在机器之间或在无服务器环境中恢复会话，请使用 [`SessionStore` 适配器#/page/agent-sdk/session-storage将记录镜像到共享存储。
...
<Note>
  Forking 分支对话历史记录，而不是文件系统。如果 forked 代理编辑文件，这些更改是真实的，对在同一目录中工作的任何会话都可见。要分支和还原文件更改，请使用[文件检查点#/page/agent-sdk/file-checkpointing。
</Note>
...
* [代理循环如何工作#/page/agent-sdk/agent-loop：了解会话中的轮次、消息和上下文累积
* [文件检查点#/page/agent-sdk/file-checkpointing：快照和还原代理在会话中所做的文件更改
