...
动态工作流是一个 JavaScript 脚本，可大规模编排[子代理#/page/sub-agents。Claude 为您描述的任务编写脚本，运行时在后台执行它，同时您的会话保持响应。
...
[子代理#/page/sub-agents、[skills#/page/skills、[agent teams#/page/agent-teams 和工作流都可以运行多步骤任务。区别在于谁掌握计划：
...
您是否看到此提示取决于您的[权限模式#/page/permission-modes：
...
* `~/.claude/workflows/` 在您的主目录中：在每个项目中可用，仅对您可见。如果您设置了 [`CLAUDE_CONFIG_DIR`#/page/env-vars，此位置是该路径下的 `workflows/` 目录。
...
主体是带有顶级 `await` 的纯 JavaScript。`agent()` 生成一个子代理，`pipeline()` 为列表中的每个项目运行一个。如果您想手动编辑脚本，要求 Claude 引导您完成更改，或查看 [Agent SDK 参考#/page/agent-sdk/typescript中的 Workflow 工具条目以获取完整的选项集。
...
工作流在 CLI、桌面应用、IDE 扩展、[非交互模式#/page/headless与 `claude -p` 和 [Agent SDK#/page/agent-sdk/overview 中可用。相同的禁用设置在每个表面上应用。
...
要为整个组织关闭工作流，在[托管设置#/page/server-managed-settings中设置 `"disableWorkflows": true`，或使用[Claude Code 管理员设置](https://claude.ai/admin-settings/claude-code)页面上的切换。
...
* [并行运行代理#/page/agents：比较子代理、代理视图、代理团队和工作流
* [创建自定义子代理#/page/sub-agents：工作流编排的工作者原语
* [管理成本#/page/costs：多代理运行如何计入使用限制
