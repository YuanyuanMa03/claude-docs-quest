...
在 PR 模式下，远程沙箱直接从主机克隆拉取请求，而不是捆绑您的本地工作树。PR 模式适用于 `github.com` 上的存储库以及 Owner 已连接到 Claude Code 的 [GitHub Enterprise Server#/page/github-enterprise-server 实例。
...
对于 GitHub 拉取请求上的自动审查，[Code Review#/page/code-review 直接与您的存储库集成，并将发现作为内联 PR 注释发布，无需 CLI 步骤。
...
* [Claude Code 网络版#/page/claude-code-on-the-web：了解远程会话和云沙箱如何工作
* [使用 ultraplan 规划复杂更改#/page/ultraplan：ultrareview 的规划对应物，用于前期设计工作
* [有效管理成本#/page/costs：跟踪使用情况并设置支出限制
