...
<Note>
  Skills 通过文件系统设置源发现。使用默认 `query()` 选项时，SDK 加载用户和项目源，因此 `~/.claude/skills/`、`<cwd>/.claude/skills/` 和 `<cwd>` 到存储库根目录之间任何父目录中的 `.claude/skills/` 中的 Skills 可用。如果显式设置 `settingSources`，请包含 `'user'` 或 `'project'` 以保持 Skill 发现，或使用 [`plugins` 选项#/page/agent-sdk/plugins 从特定路径加载 Skills。
</Note>
...
* [Claude Code 中的 Agent Skills#/page/skills：包含示例的完整指南
...
* [Claude Code 中的 Agent Skills#/page/skills：包含创建、示例和故障排除的完整 Skills 指南
...
* [SDK 中的子代理#/page/agent-sdk/subagents：具有编程选项的类似文件系统代理
* [SDK 中的 Slash Commands#/page/agent-sdk/slash-commands：用户调用的命令
* [SDK 概述#/page/agent-sdk/overview：常规 SDK 概念
* [TypeScript SDK 参考#/page/agent-sdk/typescript：完整 API 文档
* [Python SDK 参考#/page/agent-sdk/python：完整 API 文档
