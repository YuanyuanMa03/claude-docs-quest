...
[桌面应用](/docs/zh-CN/desktop#work-in-parallel-with-sessions)、[网页版 Claude Code#/page/claude-code-on-the-web 和 [VS Code 扩展](/docs/zh-CN/vs-code#resume-past-conversations)各自维护自己的会话历史记录。本页涵盖 CLI。
...
使用 [`claude -p`#/page/headless 或 [Agent SDK#/page/agent-sdk/overview 创建的会话不会出现在会话选择器中，但您仍然可以通过将其会话 ID 传递给 `claude --resume <session-id>` 来恢复它。从会话启动所在的目录运行此命令：会话 ID 查找的范围限于当前项目目录及其 git worktrees，因此在其他地方创建的会话会报告 `No conversation found with session ID: <session-id>`。
...
从 v2.1.169 开始，使用 [`/cd`#/page/commands 移动会话会将其重新定位到新目录的项目存储中，因此之后它会出现在该目录的选择器中。从 v2.1.196 开始，移动的会话在崩溃或强制退出后会保持不在旧目录的选择器中。在较早的版本中，当旧路径包含下划线等特殊字符时，在不干净的退出后，它也可能在旧目录的列表中重新出现。
...
您从未命名的交互式会话在启动时仍会获得默认显示名称。需要 Claude Code v2.1.196 或更高版本。默认名称将工作目录的名称与两个字符的后缀组合在一起，例如 `my-app-3f`，并在运行会话的列表中标识会话，例如 [agent view#/page/agent-view 和 `claude agents --json` 输出。
...
对于单个会话内基于 checkpoint 的回退，请参阅 [Checkpointing#/page/checkpointing。
...
有关压缩如何与 CLAUDE.md、skills 和规则交互的信息，请参阅[上下文窗口指南#/page/context-window。有关何时清除与压缩的策略，请参阅[最佳实践](/docs/zh-CN/best-practices#manage-your-session)。
...
* **在 TypeScript 或 Python 应用中嵌入 Claude**：使用 [Agent SDK#/page/agent-sdk/overview 以编程方式接收每条消息。
...
| 目的                | 设置                                                        | 位置                         |
| ----------------- | --------------------------------------------------------- | -------------------------- |
| 将存储移出 `~/.claude` | [`CLAUDE_CONFIG_DIR`#/page/env-vars                    | 环境变量                       |
| 更改 30 天保留期        | [`cleanupPeriodDays`](/docs/zh-CN/settings#available-settings) | `settings.json`            |
| 在所有模式下禁止文本记录写入    | [`CLAUDE_CODE_SKIP_PROMPT_HISTORY`#/page/env-vars      | 环境变量                       |
| 禁止一次非交互式运行的写入     | [`--no-session-persistence`#/page/cli-reference        | 与 `claude -p` 一起使用的 CLI 标志 |
...
* [Worktrees#/page/worktrees：在单独的分支上运行隔离的并行会话
* [Checkpointing#/page/checkpointing：将代码和对话回退到较早的点
* [Context window#/page/context-window：什么填充上下文以及什么在压缩中保留
* [Non-interactive mode#/page/headless：`claude -p` 下的会话行为
