...
要完全防止注册，在 `settings.json` 中将 [`disableDeepLinkRegistration`#/page/settings 设置为 `"disable"`。要在整个组织中强制执行此操作，使用户无法重新启用它，请改为在[托管设置#/page/server-managed-settings中设置它。
...
* [Skills#/page/skills：将长运行手册提示存储为仓库中的 `/skill`，以便深链接的 `q` 参数只需命名它
* [Non-interactive mode#/page/headless：从脚本运行 Claude 并捕获输出而不打开终端
