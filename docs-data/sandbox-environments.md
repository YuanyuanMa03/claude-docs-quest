...
<Info>
  有关更广泛的安全模型，请参阅 [Security#/page/security。有关 Agent SDK 部署，请参阅 [Secure deployment#/page/agent-sdk/secure-deployment。
</Info>
...
[Sandboxed Bash tool#/page/sandboxing 内置于 Claude Code 中，仅限制 Bash 命令。内置文件工具、MCP 服务器和 hooks 仍直接在您的主机上运行。表中的所有其他方法都将整个 Claude Code 进程放在隔离边界内，因此文件工具、MCP 服务器和 hooks 也受到限制。
...
<Warning>
  ...
  隔离也不会改变发送给模型的内容。您的提示和 Claude 读取的文件无论是否使用沙箱，都会传输到 Anthropic API 或您配置的提供商。有关 Claude Code 发送的内容以及如何减少它，请参阅 [Data usage#/page/data-usage。
</Warning>
...
| 您想要                                                      | 开始使用                                                                                          |
| :------------------------------------------------------- | :-------------------------------------------------------------------------------------------- |
| 在您自己的机器上日常工作期间减少权限提示                                     | [sandboxed Bash tool#/page/sandboxing，使用 `/sandbox` 启用                                     |
| 让 Claude 使用 `--dangerously-skip-permissions` 或自动模式无人值守工作 | 预配置的 [dev container#/page/devcontainer、任何容器或虚拟机，或 [sandbox runtime](#sandbox-runtime)      |
| 隔离 MCP 服务器和 hooks 以及 Bash，不使用 Docker                     | sandbox runtime                                                                               |
| 在不受信任的存储库上工作                                             | 专用虚拟机，或 [Claude Code on the web#/page/claude-code-on-the-web（如果您有 Claude 订阅和连接的 GitHub 账户） |
| 在团队中标准化沙箱环境                                              | 预配置的 [dev container#/page/devcontainer，复制到您的存储库中                                           |
| 从没有本地设置的设备使用 Claude Code                                 | [Claude Code on the web#/page/claude-code-on-the-web，需要 Claude 订阅和连接的 GitHub 账户            |
...
[Permission modes#/page/permission-modes 决定工具调用是否运行以及是否首先提示您。隔离限制命令运行后可以访问的内容。两者协同工作：当权限模式允许操作在不询问您的情况下运行时，隔离边界限制这些操作可以到达的内容。
...
使用 `/sandbox` 命令启用它。[Sandboxing#/page/sandboxing 指南涵盖批准模式、默认边界以及如何扩大或缩小它。

按命令沙箱不涵盖会话中运行的所有内容：

* 其他 [built-in tools#/page/tools-reference（如 Read、Edit 和 WebFetch）在 Claude Code 进程内运行，不会生成任意代码。[Permission rules#/page/permissions 用于路径或域来控制它们。
* [MCP#/page/mcp 服务器和 hooks 是在主机上无约束运行的单独进程。
...
Claude Code 存储库发布了一个 [example dev container#/page/devcontainer，其中包含默认拒绝 iptables 防火墙作为起点。将其复制到您的存储库中，并调整防火墙允许列表、基础镜像和固定的 Claude Code 版本以适应您的环境。因为防火墙阻止未批准的出站，像这样的配置支持使用 `--dangerously-skip-permissions` 运行 Claude Code 以进行无人值守工作。
...
[Claude Code on the web#/page/claude-code-on-the-web 在隔离的、由 Anthropic 管理的虚拟机中运行每个会话。网络代理强制执行默认允许列表，单独的代理在沙箱外保存您的 GitHub 令牌，同时在其内部为存储库访问发出作用域凭据。

当您想要完整的虚拟机隔离而无需自己配置基础设施，或当您从没有本地开发环境的设备委派任务时，使用此方法。它需要 Claude 订阅和连接的 GitHub 账户，会话从 GitHub 克隆您的存储库。有关计划可用性和 GitHub 身份验证选项，请参阅 [Claude Code on the web#/page/claude-code-on-the-web。
...
* **Built-in Bash sandbox**：Claude Code 强制实施的唯一方法。通过 [managed settings](/docs/zh-CN/settings#settings-files) 传递 `sandbox` 设置密钥，可以是由您的 MDM 管理的文件，也可以通过 Claude.ai 上的 [server-managed settings#/page/server-managed-settings。有关要部署的密钥以及如何防止开发人员扩大策略，请参阅 [Enforce sandboxing with managed settings](/docs/zh-CN/sandboxing#enforce-sandboxing-with-managed-settings)。
* **Dev containers**：将 [example dev container#/page/devcontainer 提交到您的存储库以在团队中标准化环境。这是一个约定而不是强制边界，因为 Claude Code 不需要容器。如果开发人员不应该能够在其外部运行 Claude Code，请使用您组织的设备管理或软件允许列表工具强制实施。
...
* [Sandboxing#/page/sandboxing：配置内置沙箱化 Bash 工具
* [Dev container#/page/devcontainer：预配置的 Docker 开发容器
* [Security#/page/security：完整的 Claude Code 安全模型
* [Secure deployment#/page/agent-sdk/secure-deployment：Agent SDK 应用程序的隔离指导
