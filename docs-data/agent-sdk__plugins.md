...
有关 plugin 结构和如何创建 plugins 的完整信息，请参阅 [Plugins#/page/plugins。

<h2 id="loading-plugins">
  加载 plugins
</h2>

通过在选项配置中提供本地文件系统路径来加载 plugins。`type` 字段必须是 `"local"`，这是 SDK 接受的唯一值。要使用通过 [marketplace#/page/plugin-marketplaces 或远程存储库分发的 plugin，请先下载它并提供本地目录路径。SDK 支持从不同位置加载多个 plugins。
...
* [Plugins#/page/plugins - 完整的 plugin 开发指南
* [Plugins reference#/page/plugins-reference - 技术规范和架构
...
* [Plugins#/page/plugins - 完整的 plugin 开发指南
* [Plugins reference#/page/plugins-reference - 技术规范
* [Commands#/page/agent-sdk/slash-commands - 在 SDK 中使用 slash commands
* [Subagents#/page/agent-sdk/subagents - 使用专门的 agents
* [Skills#/page/agent-sdk/skills - 使用 Agent Skills
