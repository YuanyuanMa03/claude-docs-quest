...
* 按用户的成本控制
* [分析#/page/analytics仪表板
* [服务器管理的设置#/page/server-managed-settings
* 审计日志
...
| Claude Code 分析 | 不存储提示或模型响应，但收集生产力元数据，如账户电子邮件和使用统计。对于 ZDR 组织，贡献指标不可用；[分析仪表板#/page/analytics仅显示使用指标。 |
...
| 功能                                                 | 原因                                |
| -------------------------------------------------- | --------------------------------- |
| [网络上的 Claude Code#/page/claude-code-on-the-web  | 需要服务器端存储对话历史。                     |
| 来自 Desktop 应用的[云会话](/docs/zh-CN/desktop#cloud-sessions) | 需要包含提示和完成的持久会话数据。                 |
| [Artifacts#/page/artifacts                      | 需要在 Anthropic 运营的基础设施上存储已发布的页面内容。 |
| 反馈提交 (`/feedback`)                                 | 提交反馈会将对话数据发送给 Anthropic。          |
| [Remote Control#/page/remote-control            | 在 Anthropic 服务器上存储会话记录以跨设备同步对话。   |
