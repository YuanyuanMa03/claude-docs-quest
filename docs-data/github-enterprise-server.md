...
对于 github.com 上的存储库，请参阅 [网络上的 Claude Code#/page/claude-code-on-the-web 和 [代码审查#/page/code-review。要在您自己的 CI 基础设施中运行 Claude，请参阅 [GitHub Actions#/page/github-actions。
...
| 贡献指标              | ✅ 支持    | 通过 webhook 传递到 [分析仪表板#/page/analytics                                                |
...
会话在 Anthropic 基础设施上运行，从 GHES 克隆您的存储库，并将更改推送回分支。使用 `/tasks` 或在 [claude.ai/code](https://claude.ai/code) 监控进度。有关完整的远程会话工作流（包括差异审查、自动修复和例程），请参阅 [网络上的 Claude Code#/page/claude-code-on-the-web。
...
有关构建市场的完整指南，请参阅 [创建和分发插件市场#/page/plugin-marketplaces。
...
如果您的组织使用 [托管设置#/page/settings 来限制开发人员可以添加哪些市场，请使用 `hostPattern` 源类型来允许来自您的 GHES 实例的所有市场，而无需枚举每个存储库：
...
* [网络上的 Claude Code#/page/claude-code-on-the-web：在云基础设施上运行 Claude Code 会话
* [代码审查#/page/code-review：自动化 PR 审查
* [插件市场#/page/plugin-marketplaces：构建和分发插件目录
* [分析#/page/analytics：跟踪使用情况和贡献指标
* [托管设置#/page/settings：组织范围的策略配置
* [网络配置#/page/network-config：防火墙和 IP 白名单要求
