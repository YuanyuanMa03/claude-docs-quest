...
| 功能                                                | 您连接到的内容                   | 何时使用                                                                   |
| :------------------------------------------------ | :------------------------ | :--------------------------------------------------------------------- |
| [Claude Code 网页版#/page/claude-code-on-the-web  | Anthropic 托管基础设施上的云会话     | 您的存储库在 GitHub 上，任务应在您放下手机后继续运行。请参阅[网页快速入门#/page/web-quickstart进行设置。 |
| [远程控制#/page/remote-control                     | 在您的计算机上运行的 Claude Code 会话 | 工作需要您的本地文件系统、工具或 MCP 服务器。                                              |
...
在应用程序中打开会话以检查进度、回答 Claude 的问题或将其引导到新的方向。您也可以告诉 Claude [监视拉取请求](/docs/zh-CN/claude-code-on-the-web#auto-fix-pull-requests)并在 CI 失败或审查评论到达时修复它们。要连接 GitHub 并设置您的环境，请按照[网页快速入门#/page/web-quickstart进行操作，并查看[Claude Code 网页版#/page/claude-code-on-the-web了解云会话可以执行的所有操作。
...
您在 Claude 应用程序中添加的附件也会到达本地会话：Claude Code 将图像或文件下载到您的机器，并将其作为 `@` 文件引用传递给 Claude。有关要求、调用模式和故障排除，请参阅[远程控制概述#/page/remote-control。
...
* [平台和集成#/page/platforms：比较 Claude Code 运行的每个表面
* [Claude Code 网页版#/page/claude-code-on-the-web：云会话如何运行以及如何在您的终端之间移动工作
* [配置云环境#/page/cloud-environments：云会话的网络访问级别、环境变量和设置脚本
* [远程控制#/page/remote-control：从任何设备继续本地会话
* [来自 Dispatch 的会话](/docs/zh-CN/desktop#sessions-from-dispatch)：Dispatch 任务如何在桌面应用程序中成为 Code 会话
* [Channels#/page/channels：通过 Telegram、Discord 或 iMessage 从您的手机询问 Claude 一些事情，同时工作在您的机器上运行
* [Slack 中的 Claude Code#/page/slack：通过提及 `@Claude` 从您的 Slack 工作区委派编码任务
