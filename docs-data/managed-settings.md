...
This page is for the administrator who deploys managed settings or debugs why one isn't applying. To decide what to enforce, start with the [Decide what to enforce](/docs/en/admin-setup#decide-what-to-enforce) table. For the claude.ai console path, see [Server-managed settings#/page/server-managed-settings. For which file a developer's own values go in, see [Settings#/page/settings.
...
The file in the steps above is one of four ways to get managed settings onto a machine. Every mechanism carries the same policy keys as a `settings.json` file, so the [settings reference#/page/settings-reference applies to all of them. A few keys are tied to particular sources, and each entry's Scope line says which:
...
| :----------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------- |
| [Server-managed settings#/page/server-managed-settings | In the claude.ai admin console, or on a self-hosted [Claude apps gateway#/page/claude-apps-gateway                                                                                                                | Fetched at startup and polled hourly; see [changes that need approval](#where-and-when-a-policy-applies)                                                                                                                                 | You want one place to change policy for a claude.ai organization without touching each machine |
...
For managed MCP servers, which you deploy alongside any of these through `managed-mcp.json`, see [Managed MCP configuration#/page/managed-mcp.

### Where and when a policy applies

A deployed policy reaches the developer's sessions as follows:

* **Surfaces**: on the developer's machine, the terminal, the VS Code and JetBrains extensions, the desktop app's Code tab, and [Agent SDK#/page/agent-sdk/typescript sessions read all of these sources. Agent SDK sessions load managed settings even when `settingSources` excludes the user, project, and local files.
* **Cloud sessions**: a session in an Anthropic-hosted environment doesn't read a device's MDM profile or file, so policy for it has to come from server-managed settings. A session in a [self-hosted environment#/page/self-hosted-environments also reads the managed settings file in its runner image, by default only when server-managed settings deliver no policy key, apart from the [keys Claude Code reads from every admin source](#keys-read-from-every-admin-source). [How Claude Code combines managed sources](#how-claude-code-combines-managed-sources) covers the opt-in that applies both.
...
* [`claude doctor`#/page/debug-your-config lists each invalid entry with its source and field.
...
| [`allowManagedMcpServersOnly`](/docs/en/settings-reference#allowmanagedmcpserversonly)                                     | When `true`, only `allowedMcpServers` from managed settings are respected. `deniedMcpServers` still merges from all sources. See [Managed MCP configuration#/page/managed-mcp                                                                                                                                                                                                                                                                                                  |
...
| [`channelsEnabled`](/docs/en/settings-reference#channelsenabled)                                                           | Allow [channels#/page/channels for the organization. See [enterprise controls](/docs/en/channels#enterprise-controls) for the default on each plan                                                                                                                                                                                                                                                                                                                                  |
...
<Note>
  On Team and Enterprise plans, an Owner enables or disables [Remote Control#/page/remote-control and [web sessions#/page/claude-code-on-the-web organization-wide in [Claude Code admin settings](https://claude.ai/admin-settings/claude-code). Remote Control can additionally be disabled per device with the [`disableRemoteControl`](/docs/en/settings-reference#disableremotecontrol) setting. Web sessions have no per-device managed settings key.
</Note>
...
* [Set up Claude Code for your organization#/page/admin-setup: decide what to enforce and how
* [Server-managed settings#/page/server-managed-settings: deliver policy from the claude.ai console or a gateway
* [Managed MCP configuration#/page/managed-mcp: control which MCP servers developers can use
* [Settings reference#/page/settings-reference: every key, with whether a managed source can set it
