...
* 打开的终端或命令提示符
  * 如果您之前从未使用过终端，请查看[终端指南#/page/terminal-guide
* 一个可以使用的代码项目
* 一个 [Claude 订阅](https://claude.com/pricing?utm_source=claude_code\&utm_medium=docs\&utm_content=quickstart_prereq)（Pro、Max、Team 或 Enterprise）、[Claude Console](https://console.anthropic.com/) 账户，或通过[支持的云提供商#/page/third-party-integrations的访问权限

<Note>
  本指南涵盖终端 CLI。Claude Code 也可在[网页](https://claude.ai/code)、[桌面应用#/page/desktop、[VS Code#/page/vs-code 和 [JetBrains IDE#/page/jetbrains、[Slack#/page/slack 中使用，以及通过 [GitHub Actions#/page/github-actions 和 [GitLab#/page/gitlab-ci-cd 进行 CI/CD。查看[所有界面](/docs/zh-CN/overview#use-claude-code-everywhere)。
</Note>
...
* [Amazon Bedrock、Google Cloud 的 Agent Platform 或 Microsoft Foundry#/page/third-party-integrations（企业云提供商）
* 自托管的 [Claude apps gateway#/page/claude-apps-gateway（如果您的组织运行一个）：您的管理员会预先配置网关 URL，`/login` 会直接在 **Cloud gateway** 屏幕上打开，供您使用企业 SSO 登录
...
有关完整的 shell 命令列表，请参阅 [CLI 参考#/page/cli-reference，有关完整的会话命令列表，请参阅 [命令参考#/page/commands。

<h2 id="pro-tips-for-beginners">
  初学者专业提示
</h2>

有关更多信息，请参阅[最佳实践#/page/best-practices和[常见工作流#/page/common-workflows。
