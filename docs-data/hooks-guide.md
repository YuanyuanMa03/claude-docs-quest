...
有关扩展 Claude Code 的其他方式，请参阅 [skills#/page/skills 用于为 Claude 提供额外的指令和可执行命令，[subagents#/page/sub-agents 用于在隔离的上下文中运行任务，以及 [plugins#/page/plugins 用于打包要在项目间共享的扩展。

<Tip>
  本指南涵盖常见用例和入门方法。有关完整的事件架构、JSON 输入/输出格式和异步 hooks 和 MCP 工具 hooks 等高级功能，请参阅 [Hooks 参考#/page/hooks。
</Tip>
...
| `agent_needs_input`    | 后台会话开始等待你的输入。仅在 [agent view#/page/agent-view 打开时触发 |
| `agent_completed`      | 后台会话完成或失败。仅在 [agent view#/page/agent-view 打开时触发    |
...
你可以用任何产生动态输出的命令替换 `echo`，如 `git log --oneline -5` 来显示最近的提交。有关在每个会话开始时注入上下文，请考虑改用 [CLAUDE.md#/page/memory。有关环境变量，请参阅参考中的 [`CLAUDE_ENV_FILE`](/docs/zh-CN/hooks#persist-environment-variables)。
...
* **退出 0**：hook 报告没有异议，操作正常进行。对于 `PreToolUse` hook，这不会批准工具调用：正常的 [权限流程#/page/permissions 仍然适用。对于 `UserPromptSubmit`、`UserPromptExpansion` 和 `SessionStart` hooks，你写入 stdout 的任何内容都会添加到 Claude 的上下文中。
...
第四个值 `"defer"` 在 [非交互模式#/page/headless 中使用 `-p` 标志时可用。它以保留的工具调用退出进程，以便 Agent SDK 包装器可以收集输入并恢复。请参阅参考中的 [延迟工具调用以供稍后使用](/docs/zh-CN/hooks#defer-a-tool-call-for-later)。
...
`if` 字段使用 [权限规则语法#/page/permissions 按工具名称和参数一起过滤 hooks，因此 hook 进程仅在工具调用匹配时生成。这超越了 `matcher`，它仅在工具名称级别按组过滤。
...
当 Bash 命令无法解析时，过滤器也会失败开放，无论如何都会运行你的 hook。因为过滤器是尽力而为的，使用 [权限系统#/page/permissions 而不是 hook 来强制执行硬允许或拒绝。
...
| [Skill#/page/skills 或 [agent#/page/sub-agents frontmatter | 当 skill 或 agent 处于活动状态时 | 是，在组件文件中定义                      |
...
* `PermissionRequest` hooks 不在 [非交互模式#/page/headless（带 `-p` 标志）中触发。对于自动化权限决策，使用 `PreToolUse` hooks。
...
如果你的 hook 合理地需要超过八次迭代才能收敛，使用 [`CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`#/page/env-vars 提高上限。
...
* [Hooks 参考#/page/hooks：完整的事件架构、JSON 输出格式、异步 hooks 和 MCP 工具 hooks
