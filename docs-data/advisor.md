...
* **`advisorModel` 设置**：在您的[设置文件#/page/settings中配置持久默认值
...
在会话中途启用或禁用顾问不会使主模型的[提示缓存#/page/prompt-caching失效。与[更改模型或努力级别](/docs/zh-CN/prompt-caching#actions-that-invalidate-the-cache)不同，切换 `/advisor` 会保持缓存的前缀完整，顾问返回的指导在后续轮次中作为成绩单的一部分被缓存。
...
* **仅 Anthropic API**：顾问是服务器执行的工具。它在 Amazon Bedrock、Claude Platform on AWS、Google Cloud 的 Agent Platform 或 Microsoft Foundry 上不可用。通过配置了 `ANTHROPIC_BASE_URL` 的 [LLM 网关#/page/llm-gateway，可用性取决于网关是否将请求完整转发到 Anthropic API。
...
要完全禁用顾问工具，设置 `CLAUDE_CODE_DISABLE_ADVISOR_TOOL=1`。`/advisor` 命令变为不可用，任何配置的 `advisorModel` 都会被忽略。`--advisor` 标志被接受但没有效果；传递它的现有脚本继续工作而不会出现错误。请参阅[环境变量#/page/env-vars。
...
* [模型配置#/page/model-config：切换模型、设置努力级别并使用 `opusplan`
* [有效管理成本#/page/costs：跨模型跟踪令牌使用情况
