...
<Note>
  Self-hosted environments are in public beta on Team and Enterprise plans; an [Owner](/docs/en/cloud-environments#organization-shared-environments) enables them by turning on **Allow self-hosted environments** on the [**Cloud environments** admin page](https://claude.ai/admin-settings/cloud-environments). This page covers session identity verification; see the [quickstart#/page/self-hosted-environments-quickstart for setup and [Deploy to production#/page/self-hosted-environments-deploy for the fleet recipes.
</Note>

A [self-hosted environment#/page/self-hosted-environments lets [Claude Code on the web#/page/claude-code-on-the-web sessions run on infrastructure you operate instead of on Anthropic's. Because the session runs inside your network, Claude can call your internal services directly. Those services need a way to confirm that a request came from a Claude Code session in your environment, and to identify the user or service identity that created that session.
...
* [Self-hosted environments#/page/self-hosted-environments: the environment, runner, and session model; the [quickstart#/page/self-hosted-environments-quickstart and [Deploy to production#/page/self-hosted-environments-deploy hold setup and operations
* [Customize sessions#/page/self-hosted-environments-configuration: wrapper scripts that consume the token, and the `spawn-runner` hook
* [Reference#/page/self-hosted-environments-reference: CLI flags, environment variables, and metrics
