...
<Tip>
  本页面涵盖输出流式传输（实时接收令牌）。有关输入模式（如何发送消息），请参阅[向代理发送消息#/page/agent-sdk/streaming-vs-single-mode。您也可以[通过 CLI 使用 Agent SDK 流式传输响应#/page/headless。
</Tip>
...
* **结构化输出**：JSON 结果仅出现在最终 `ResultMessage.structured_output` 中，而不是作为流式增量。有关详细信息，请参阅[结构化输出#/page/agent-sdk/structured-outputs。
...
* [交互式与一次性查询#/page/agent-sdk/streaming-vs-single-mode：为您的用例选择输入模式
* [结构化输出#/page/agent-sdk/structured-outputs：从代理获取类型化的 JSON 响应
* [权限#/page/agent-sdk/permissions：控制代理可以使用哪些工具
