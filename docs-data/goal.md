...
[Auto mode#/page/auto-mode-config 本身在单个回合内批准工具调用，但不会启动新的回合。Claude 在判断工作完成时停止。`/goal` 添加了一个单独的评估器，在每个回合后检查你的条件，因此完成由一个新鲜的模型而不是执行工作的模型决定。这两者是互补的：auto mode 消除了每个工具的提示，`/goal` 消除了每个回合的提示。
...
目标不会改变权限。在默认权限模式下，Claude 在进行工具调用前仍会询问，这些工具调用是你的设置不允许的，例如上面的测试命令。要让目标回合无人值守地运行，请将 `/goal` 与[自动模式#/page/auto-mode-config配对。
...
`/goal` 在[非交互式模式#/page/headless、[桌面应用#/page/desktop中工作，并通过[远程控制#/page/remote-control工作。使用 `-p` 设置目标会在单个调用中运行循环至完成：
...
`/goal` 是会话范围的[基于提示的 Stop hook](/docs/zh-CN/hooks#prompt-based-hooks)的包装器。每次 Claude 完成一个回合时，条件和到目前为止的对话都会发送到你配置的[小型快速模型#/page/model-config，默认为 Haiku。模型返回一个是或否的决定和一个简短的原因。"否"告诉 Claude 继续工作，并包括原因作为下一个回合的指导。"是"清除目标并在记录中记录一个已实现的条目。
...
* [自动模式#/page/auto-mode-config：自动批准工具调用，以便每个目标回合无人值守运行
