...
本页面涵盖系统要求、特定平台安装详情、更新和卸载。有关首次会话的引导式演练，请参阅[快速入门#/page/quickstart。如果您从未使用过终端，请参阅[终端指南#/page/terminal-guide。
...
<Tip>
  更喜欢图形界面？[桌面应用#/page/desktop-quickstart让您无需使用终端即可使用 Claude Code。下载适用于 [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs)、[Windows](https://claude.com/download?utm_source=claude_code\&utm_medium=docs) 或 [Linux#/page/desktop-linux 的版本。

  初次使用终端？请参阅[终端指南#/page/terminal-guide获取分步说明。
</Tip>
...
如果在安装过程中遇到任何问题，请参阅[故障排除安装和登录#/page/troubleshoot-install。
...
* **使用 Git for Windows**，Claude Code 为 [Bash 工具](/docs/zh-CN/tools-reference#bash-tool-behavior)使用 Git Bash。如果 Claude Code 找不到 Git Bash，请在您的 [settings.json 文件#/page/settings中设置路径：
...
如果此命令失败并显示 `command not found` 或其他错误，请参阅[排查安装和登录问题#/page/troubleshoot-install。
...
Claude Code 需要 Pro、Max、Team、Enterprise 或 Console 账户。免费的 Claude.ai 计划不包括 Claude Code 访问权限。您也可以通过第三方 API 提供商（如 [Amazon Bedrock#/page/amazon-bedrock、[Google Cloud's Agent Platform#/page/google-vertex-ai 或 [Microsoft Foundry#/page/microsoft-foundry）使用 Claude Code。

安装后，通过运行 `claude` 并按照浏览器提示登录。有关所有账户类型和团队设置选项，请参阅[身份验证#/page/authentication。
...
<Note>
  ...
  要让 Claude Code 在 Homebrew 或 WinGet 上为您运行升级命令，请将 [`CLAUDE_CODE_PACKAGE_MANAGER_AUTO_UPDATE`#/page/env-vars 设置为 `1`。Claude Code 随后会在新版本可用时在后台运行升级，并在成功时显示重启提示。升级仅针对 Claude Code 包，不会影响您已安装的其他软件。
...
通过 `/config` → **自动更新渠道**配置此项，或将其添加到您的 [settings.json 文件#/page/settings：
...
将其添加到您的 [settings.json 文件#/page/settings以显式固定下限：
...
`DISABLE_AUTOUPDATER` 仅停止后台检查；`claude update` 和 `claude install` 仍然有效。要阻止所有更新路径（包括手动更新），请改为设置 [`DISABLE_UPDATES`#/page/env-vars。当您通过自己的渠道分发 Claude Code 并需要用户保持在您提供的版本上时，请使用此选项。
