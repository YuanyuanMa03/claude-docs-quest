...
| 可选的配对或演练    | 0 到 30 分钟 | 为真正被阻挡的同事保留此项，并在安排时间之前提供 [Quickstart#/page/quickstart 链接。 |
...
| 问题              | 建议的回应                                                                     | 后续资源                                                          |
| --------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------- |
| "我应该首先在什么上尝试它？" | 推荐一个真实但有限的任务，最好是一个你一直在推迟的错误或琐事，因为它很繁琐而不是困难。                               | [Common workflows#/page/common-workflows                   |
| "我如何相信它处理我的代码？" | 介绍 plan mode：按 `Shift+Tab` 循环进入它，Claude 准确地提议它打算改变什么，在用户批准之前不会修改任何东西。     | [Permissions#/page/permissions                             |
| "设置值得付出努力吗？"    | 安装大约需要两分钟，在终端中运行，不需要 IDE 扩展。运行一次 `/init` 足以开始工作。                          | [Quickstart#/page/quickstart                               |
| "它产生了不正确的结果。"   | 鼓励他们将失败提供给 Claude。粘贴错误消息或失败的测试远比重新表述原始请求更有效。                              | [Common workflows#/page/common-workflows                   |
| "它不理解我们的代码库约定。" | 建议运行 `/init` 生成 `CLAUDE.md` 文件，然后添加团队的约定、测试命令和任何应该避免的目录。                  | [Memory#/page/memory                                       |
| "这只是自动完成吗？"     | 提供一个简短的演示，其中 Claude 解释一个不熟悉的文件、跨服务追踪一个错误或起草迁移计划。这些任务需要在存储库中进行推理，而不是完成单一行。 | 一个两分钟的现场演示                                                    |
| "安全和数据处理呢？"     | 将此问题转介给你的管理员。你的组织的部署和数据处理政策已经配置，倡导者不应该即兴回答这个问题。                           | [Security#/page/security · [Data usage#/page/data-usage |
...
| 模式            | 如何运行它                                                                                                         | 所需的努力          |
| ------------- | ------------------------------------------------------------------------------------------------------------- | -------------- |
| 专用频道          | 创建一个 `#claude-code` 频道（或现有频道中的定期线程），固定 [Quickstart#/page/quickstart 链接和一个强大的例子，并公开回答问题，以便每个答案都能使观看的每个人受益。  | 大约五分钟的设置，然后是环境 |
...
<Steps>
  <Step title="第 1 周：为频道播种">
    创建频道，固定 [Quickstart#/page/quickstart，并发布两三个你自己的例子，包括提示。
...
你是温暖的介绍而不是入职计划。当同事从"我应该尝试这个吗"进入"我如何有效地使用它"时，将他们指向 [Quickstart#/page/quickstart 和 [Common workflows#/page/common-workflows 页面。它们包含涵盖真正有用但难以自己发现的功能的短部分。
...
| 教它你的存储库    | 运行 `/init` 生成 `CLAUDE.md` 文件，然后添加你的约定、测试命令和任何不应该修改的目录。参见 [Memory#/page/memory。               |
| 重用工作流      | 在 `.claude/skills/<name>/` 中保存 `SKILL.md` 文件以创建整个团队可以使用的 `/name` 技能。参见 [Skills#/page/skills。 |
| 在长任务期间保持知情 | 配置一个 Stop hook 以在长时间运行的任务完成时收到桌面通知。参见 [Hooks#/page/hooks-guide。                              |
| 从不正确的结果中恢复 | 与其重新表述请求，不如将失败的测试或堆栈跟踪粘贴回 Claude，并要求它解决该特定失败。                                                   |
| 保持编辑手术性    | 要求一个 diff，或指定"仅改变 X。"Claude 在陈述范围时尊重范围。                                                         |

<Tip>
  Claude Code 经常更新。在内部分发此材料之前，根据 [documentation home page#/page/overview 验证版本特定的详细信息。
</Tip>
