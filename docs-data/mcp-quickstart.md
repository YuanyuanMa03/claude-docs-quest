...
有关在 Claude Code 中连接和配置 MCP 服务器的所有方式，请参阅 [MCP 参考#/page/mcp。
...
* [已安装#/page/quickstart并验证了 Claude Code
...
在 Windows 上，`~/.claude.json` 解析为 `%USERPROFILE%\.claude.json`，通常为 `C:\Users\YourName\.claude.json`。如果您设置了 [`CLAUDE_CONFIG_DIR`#/page/env-vars，Claude Code 会从该目录内读取 `.claude.json`。
...
<AccordionGroup>
  ...
  <Accordion title="连接在启动时超时">
    服务器花费的时间超过了默认的 30 秒启动超时。stdio 服务器的第一次运行在 `npx` 下载包时可能很慢。使用 [`MCP_TIMEOUT`#/page/env-vars 环境变量（以毫秒为单位）增加限制：
...
* [为组织管理 MCP 访问#/page/managed-mcp 使用托管设置和策略控制
