...
* **基于文件系统**：在 `.claude/agents/` 目录中将代理定义为 markdown 文件。请参阅[将子代理定义为文件#/page/sub-agents
...
* 子代理默认在后台运行。省略 [`run_in_background`#/page/agent-sdk/typescript 输入的 Agent 工具调用会启动后台子代理，当 Claude 需要结果后才继续时，它会设置 `run_in_background: false`。在 v2.1.198 之前，省略 `run_in_background` 会同步运行子代理。设置 `background` 字段为 `true` 以强制特定代理进行后台执行，无论 Claude 请求什么。
...
您也可以在 `.claude/agents/` 目录中将子代理定义为 markdown 文件。有关此方法的详细信息，请参阅 [Claude Code 子代理文档#/page/sub-agents。以编程方式定义的代理优先于具有相同名称的基于文件系统的代理。
...
具有 [`SendMessage`#/page/tools-reference 工具的子代理会从会话中运行的其他命名代理列表开始，因此它知道可以向哪些名称发送消息。Claude Code 会自动在子代理的第一轮中添加该列表。[fork](/docs/zh-CN/sub-agents#fork-the-current-conversation) 不会获得该列表，因为它继承了父对话。该列表需要 Claude Code v2.1.206 或更高版本。
...
子代理适用于每轮委派的几个任务。对于协调数十到数百个代理的运行，请使用 `Workflow` 工具，它将编排移到运行时在对话上下文外执行的脚本中。请参阅[动态工作流#/page/workflows以了解工作流与逐轮子代理委派的区别。
...
* [Claude Code 子代理#/page/sub-agents：包括基于文件系统的定义的全面子代理文档
* [动态工作流#/page/workflows：从脚本编排许多子代理，用于对话过大的工作
* [SDK 概述#/page/agent-sdk/overview：Claude Agent SDK 入门
